...
export interface UploadState {
    ...
    // We can't store the File object effectively in global state if we want persistence,
    // but for context it's fine. For simplicity, we just track status/progress/metadata.
    // The actual File might be lost on refresh, but component unmount is fine.
    previewData: any[];
}
...
interface GlobalContextType {
    ...
    setUploadState: (updates: Partial<UploadState>) => void;
}
...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const setUploadState = useCallback((updates: Partial<UploadState>) => {
        setState(prev => ({
            ...prev,
            uploadState: { ...prev.uploadState, ...updates }
        }));
    }, []);
    ...
    return (
        <GlobalContext.Provider value={{ state, dispatchEvent, updateLoad, upgradeSystem, setDefcon, setUploadState }}>
            {children}
    ...
    if (context === undefined) {
        ...
        return {
            state: {
                ...
                activeThreats: 0,
                isMaintenanceMode: false,
                uploadState: { status: 'idle' as const, progress: 0, fileName: null, fileSize: null, previewData: [] }
            },
            dispatchEvent: () => {},
            updateLoad: () => {},
            upgradeSystem: () => {},
