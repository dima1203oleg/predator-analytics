...
rsync -avz -e "ssh -p ${SSH_PORT}" --exclude 'node_modules' --exclude '__pycache__' --exclude '.git' ../deploy ../services ${USER}@${SERVER_IP}:${REMOTE_DIR}/
...
ssh -p ${SSH_PORT} ${USER}@${SERVER_IP} "cd ${REMOTE_DIR}/deploy && docker compose exec graph-service pip install asyncpg && docker compose exec graph-service python /app/deploy/scripts/seed.py"
