...
REMOTE="dmytrokizima@194.177.1.240"
CLUSTER_NAME="predator-sovereign"

echo "🚀 Починаю автономне розгортання на NVIDIA ($REMOTE)..."

# 1. Перевірка Docker
if ! ssh $REMOTE "docker ps" > /dev/null 2>&1; then
    echo "❌ Docker не запущений на NVIDIA. Спробую запустити colima..."
...
echo "📂 Синхронізація чартів на NVIDIA..."
...
ssh $REMOTE "helm upgrade --install predator ~/predator-deploy/helm/predator \
    ...
    -f ~/predator-deploy/helm/predator/values-NVIDIA.yaml"

# 6. Тестування доступності API
echo "🔍 Тестування API-шлюзу..."
sleep 15
if curl -s http://194.177.1.240:8000/api/v1/health | grep "status" > /dev/null; then
    echo "✅ Backend доступний на NVIDIA!"
else
