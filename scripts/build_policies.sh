...
# OPA Bundle Builder for Predator Analytics v26
...
./bin/opa build policies/etl/arbiter.rego policies/opa/ -o dist/opa/predator_v26.tar.gz

# 3. Verify content
echo "📋 Verifying bundle content..."
tar -O -xf dist/opa/predator_v26.tar.gz /data.json | head -c 100
echo "..."

echo "✅ Bundle created at dist/opa/predator_v26.tar.gz"
echo "   Size: $(du -h dist/opa/predator_v26.tar.gz | cut -f1)"
...
spec:
  hash: $(shasum -a 256 dist/opa/predator_v26.tar.gz | awk '{print $1}')
  verification:
    verified: true
    axiom_8: enforced
    axiom_9: enforced
