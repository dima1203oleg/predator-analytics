...
Запуск в Kaggle:
1. Завантаж цей файл в Kaggle Notebook
2. Запусти: python predator_kaggle_backend_v65.py
...
if __name__ == "__main__":
    # Перевірка чи запущено в Jupyter/Kaggle
    try:
        import nest_asyncio
        nest_asyncio.apply()
        asyncio.run(main())
    except ImportError:
        # Якщо nest_asyncio не встановлено, спробувати звичайний запуск
        try:
            asyncio.run(main())
        except RuntimeError as e:
            if "asyncio.run() cannot be called from a running event loop" in str(e):
                # Запущено в Jupyter без nest_asyncio
                print("Виявлено Jupyter/Kaggle середовище. Використовуйте:")
                print("1. !pip install nest_asyncio")
                print("2. import nest_asyncio; nest_asyncio.apply()")
                print("3. await main()")
            else:
                raise
