...
if command -v docker &> /dev/null; then
    echo "Використання Docker ($CONTAINER_NAME)..."
...
if command -v docker &> /dev/null; then
    EXISTING_INDEXES=$(docker exec $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM pg_indexes WHERE indexname LIKE 'idx_%';")
else
    EXISTING_INDEXES=$(PGPASSWORD=$DB_PASSWORD psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM pg_indexes WHERE indexname LIKE 'idx_%';")
fi

# Trim whitespace
EXISTING_INDEXES=$(echo $EXISTING_INDEXES | xargs)
...
if command -v docker &> /dev/null; then
    NEW_INDEXES=$(docker exec $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM pg_indexes WHERE indexname LIKE 'idx_%';")
else
    NEW_INDEXES=$(PGPASSWORD=$DB_PASSWORD psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM pg_indexes WHERE indexname LIKE 'idx_%';")
fi
