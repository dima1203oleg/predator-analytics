        ...
        <div className="fixed inset-0 bg-[#020617] flex flex-col items-center justify-center z-[90] p-4 pb-safe overflow-hidden">
            ...
            <div className="w-full max-w-md relative z-10 animate-in zoom-in-95 duration-500">
                ...
                <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-700 rounded-2xl shadow-2xl relative overflow-hidden panel-3d">
                    ...
                    <div className="p-8 space-y-8">
                        <div className="text-center relative">
                            ...
                            <p className="text-xs text-primary-400 font-mono tracking-[0.3em] uppercase mt-1 opacity-80">Безпечний Доступ v18.6</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 p-1.5 bg-slate-950/50 rounded-lg border border-slate-800">
                            <button
                                ...
                                className={`py-2.5 text-[10px] font-bold rounded-md flex items-center justify-center gap-2 transition-all btn-3d ${method === 'TOKEN'
                                        ? 'bg-primary-900/20 border border-primary-500/50 text-primary-400 shadow-[0_0_10px_rgba(6,182,212,0.2)]'
                                        : 'text-slate-500 hover:text-slate-300 hover:bg-slate-900'
                                    }`}
                            >
                                <Key size={14} /> ТОКЕН ДОСТУПУ
                            </button>
                            <button
                                onClick={() => setMethod('SSO')}
                                className={`py-2.5 text-[10px] font-bold rounded-md flex items-center justify-center gap-2 transition-all btn-3d ${method === 'SSO'
                                        ? 'bg-purple-900/20 border border-purple-500/50 text-purple-400 shadow-[0_0_10px_rgba(168,85,247,0.2)]'
                                        : 'text-slate-500 hover:text-slate-300 hover:bg-slate-900'
                                    }`}
                            >
                                <Globe size={14} /> GOV.ID ВХІД
                            </button>
                        </div>
                        ...
                        {method === 'TOKEN' ? (
                            <form onSubmit={handleLogin} className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-slate-400 uppercase ml-1 tracking-wider flex items-center gap-2">
                                        <Scan size={12} className="text-primary-500" /> Ключ Доступу
                                    </label>
                                ...
                                <button
                                    ...
                                    className={`w-full py-3.5 font-bold rounded-lg shadow-lg flex items-center justify-center gap-3 transition-all btn-3d ${isLoading || !token
                                ...
                                >
                                    {isLoading ? (
                                        <><RefreshCw size={18} className="animate-spin" /> ПЕРЕВІРКА...</>
                                    ) : (
                                        <>АВТОРИЗАЦІЯ <ArrowRight size={18} /></>
                                    )}
                                </button>
                            </form>
                        ) : (
                            <div className="space-y-6 text-center py-4 animate-in fade-in">
                                ...
                                <p className="text-xs text-slate-400 font-mono">
                                    Перенаправлення до Єдиного Національного Провайдера Ідентифікації...<br />
                                    <span className="text-purple-400">(NBU BankID / Дія.Підпис)</span>
                                </p>
                                <button
                                    onClick={handleLogin}
                                ...
                                >
                                    ...
                                    {isLoading ? "З'ЄДНАННЯ..." : "УВІЙТИ ЧЕРЕЗ SSO"}
                                </button>
                            </div>
                        )}
                        ...
                        <div className="text-center space-y-3 pt-4 border-t border-slate-800/50">
                            <div className="flex justify-center gap-6 text-[9px] text-slate-500 font-mono uppercase tracking-widest">
                                <span className="flex items-center gap-1.5"><Shield size={10} className="text-green-500" /> TLS 1.3 Шифрування</span>
                                <span className="flex items-center gap-1.5"><Cpu size={10} className="text-blue-500" /> SGX Анклав</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="text-center mt-8 text-[10px] text-slate-600 font-mono">
                    &copy; 2023-2025 Predator Analytics. Обмежений Доступ.
                </div>
            </div>
        </div>
    );
