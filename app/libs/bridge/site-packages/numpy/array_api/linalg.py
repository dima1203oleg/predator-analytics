...
# literals.
def matrix_norm(x: Array, /, *, keepdims: bool = False, ord: int |  float, Literal['fro', 'nuc' | None] = 'fro') -> Array:
    """
...
def svdvals(x: Array, /) -> Array |  Tuple[Array, ...]:
    if x.dtype not in _floating_dtypes:
...
def tensordot(x1: Array, x2: Array, /, *, axes: int |  Tuple[Sequence[int, Sequence[int]]] = 2) -> Array:
    # Note: the restriction to numeric dtypes only is different from
    # np.tensordot.
    if x1.dtype not in _numeric_dtypes or x2.dtype not in _numeric_dtypes:
...
def vector_norm(x: Array, /, *, axis: int |  Tuple[int, ... | None] = None, keepdims: bool = False, ord: int |  float | None = 2) -> Array:
    """
    Array API compatible wrapper for :py:func:`np.linalg.norm <numpy.linalg.norm>`.
