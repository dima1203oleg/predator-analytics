...
@registry.register(
    name="kube_ctl", description="Run kubectl commands to check or manage Kubernetes resources"
)
def kube_ctl(command: str, namespace: str = "default") -> str:
    ...
    try:
        ...
        base_cmd = command.split(maxsplit=1)[0]
...
@registry.register(
    name="get_container_status", description="Get status of all Predator Docker containers"
)
...
@registry.register(
    name="system_restart", description="Emergency restart of all Predator Analytics services"
)
def system_restart() -> str:
    ...
    try:
        ...
        result = subprocess.run(
            ["docker-compose", "restart"], check=False, capture_output=True, text=True, timeout=60
        )
        return result.stdout if result.returncode == 0 else result.stderr
    except Exception as e:
        return f"Exception: {e!s}"
...
@registry.register(
    name="system_rollback", description="Rollback codebase to the last stable state (Git)"
)
...
@registry.register(
    name="system_doctor", description="Run full system health check (Docker, DB, Redis, LLM)"
)
def system_doctor() -> str:
    ...
    try:
        ...
        result = subprocess.run(
            ["bash", script_path], check=False, capture_output=True, text=True, timeout=30
        )
        return result.stdout
    except Exception as e:
        return f"Exception: {e!s}"
...
@registry.register(
    name="fix_issue", description="Apply automated fixes for identified system issues"
)
def fix_issue(issue_type: str) -> str:
    ...
    try:
        ...
        return f"❌ Fix failed for {issue_type}: {e!s}"
