...
class ETLTaskPayload(BaseModel):
    ...
    options: dict[str, Any] = Field(
        ...
        description="Processing options (validate_schema, chunk_size, etc)"
    )
...
class MLTrainingPayload(BaseModel):
    ...
    training_config: dict[str, Any] | None = Field(
        None,
        description="Advanced training config (epochs, batch_size, etc)"
    )
    auto_deploy: bool = Field(
        default=False,
        description="Auto-deploy if accuracy > threshold"
    )
