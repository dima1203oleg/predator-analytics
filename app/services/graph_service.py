...
class GraphBuilderService:
    ...
    async def extract_and_build(self, doc_id: str, text: str, tenant_id: str):
        ...
        try:
            ...
            logger.exception(f"Graph extraction error: {e}")
    ...
    def _parse_llm_json(self, content: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Failed to parse JSON: {e} - Content: {content[:100]}...")
            return {}
            ...
            try:
                ...
                try:
                    for node in data.get("nodes", []):
                        if node.get("label") in ["ORGANIZATION", "COMPANY"]:
                            name = node.get("name", "Unknown")
                            info = node.get("properties", {})
                            code = info.get("code") or info.get("edrpou") or name
                            await graph_db.sync_company(name=name, code=code)
                except Exception as ne:
                    logger.warning(f"Neo4j sync partially failed: {ne}")

            except Exception as e:
                await session.rollback()
                logger.exception(f"DB persistence failed: {e}")
                raise
        ...
        async with async_session_maker() as session:
            try:
                ...
                stmt = text("""
                    ...
                    GROUP BY label
                """)
                result = await session.execute(stmt, {"tenant_id": uuid.UUID(tenant_id)})
                label_counts = {row[0]: row[1] for row in result.fetchall()}

                # Total counts
                total_nodes = sum(label_counts.values())

                stmt_edges = text("SELECT COUNT(*) FROM graph_edges")
