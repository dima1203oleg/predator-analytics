...
from app.libs.core.models.entities import DataSource, ETLJob, MLDataset
...
class ETLIngestionService:
    ...
    async def _update_job_state(self, job_id: uuid.UUID, state: ETLState, context: dict[str, Any] | None = None, error: str | None = None):
    ...
    async def process_file(self, file_path: str, dataset_type: str = "customs") -> dict[str, Any]:
        ...
        is_large_excel = file_path.endswith(".xlsx") or file_path.endswith(".xls")
        ...
        try:
            ...
            if is_large_excel:
                ...
                await self._update_job_state(job_id, ETLState.PROCESSING, {"progress": {"message": "Initializing Stream..."}})

                # Streaming Flow: Read Chunk -> Transform -> Distribute
                async for batch_idx, df_chunk in self._read_excel_batched(file_path, str(job_id), chunk_size=2000):
                    ...
                    await self._update_job_state(job_id, ETLState.PROCESSING, {
                        "progress": {
                            ...
                            "percent": min(95, int((total_records_processed / 1000000) * 100)) # Fake percent if total unknown, or updated if we knew total
                ...
                await self._update_job_state(job_id, ETLState.INDEXING, {
                     "progress": {"records_processed": len(records)}
                })
