...
class MonitoringService:
    ...
    async def get_system_metrics(self) -> dict[str, Any]:
        """Fetch real metrics from Prometheus or local psutil fallback."""
        metrics = {
            "cpu_load": 0.0,
            ...
            async with httpx.AsyncClient(timeout=2) as client:
                ...
                if cpu_resp.status_code == 200:
                    ...
                    if data.get("data", {}).get("result"):
                        val = round(float(data["data"]["result"][0]["value"][1]), 2)
        ...
        if metrics["status"] == "offline":
            try:
                ...
                metrics["memory_usage"] = ram
                metrics["ram_usage"] = ram
                metrics["status"] = "local"
            except Exception:
                pass
    ...
    async def get_queue_status(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(f"RabbitMQ API unreachable: {e}")
