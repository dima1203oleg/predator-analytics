...
class TripleAgentService(TrinityCore):
    ...
    def __init__(self):
        ...
        from app.libs.core.secrets import secret_manager
    ...
    async def generate_code(self, plan: str) -> str:
        ...
        if self.cli_stack:
            try:
                plan_dict = self._current_plan_raw or {"description": plan}
                self._current_code_raw = {"code": self.cli_stack.codegen_agent(plan_dict)}
                return self._current_code_raw.get("code", "")
            except Exception as e:
                logger.exception(f"Mistral Vibe CLI Codegen failed: {e}")
