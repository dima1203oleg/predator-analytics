...
HTTP_REQUEST_SIZE = _get_or_create_metric(
    Histogram, "http_request_size_bytes", "HTTP request size in bytes", ["method", "endpoint"]
)

HTTP_RESPONSE_SIZE = _get_or_create_metric(
    Histogram, "http_response_size_bytes", "HTTP response size in bytes", ["method", "endpoint"]
)
...
RATE_LIMIT_EXCEEDED = _get_or_create_metric(
    Counter, "rate_limit_exceeded_total", "Number of rate limit exceeded responses", ["client_ip"]
)
