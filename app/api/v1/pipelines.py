...
async def list_pipeline_jobs(
    ...
    try:
        return await data_hub_service.list_jobs(
            dataset_id=dataset_id, job_type=pipeline_type, status=status, limit=limit, offset=offset
        )
    except Exception as e:
...
async def get_pipeline_job(job_id: UUID, current_user: User = Depends(get_current_user)):
...
async def get_job_logs(job_id: UUID, current_user: User = Depends(get_current_user)):
...
async def cancel_pipeline_job(job_id: UUID, current_user: User = Depends(get_current_user)):
...
async def retry_pipeline_job(job_id: UUID, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        update_data = JobUpdate(status=JobStatus.QUEUED, progress=0.0, error_message=None)
        job = await data_hub_service.update_job(job_id, update_data)
...
async def get_pipeline_definitions(current_user: User = Depends(get_current_user)):
