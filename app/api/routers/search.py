...
try:
    ...
    logger = get_logger("predator.api.search")
...
async def search(
    ...
    try:
        ...
        if mode == "hybrid":
            ...
            if rerank and top_candidates:
                try:
                    ...
                    logger.exception("reranking_failed", error=str(e))
                    # import traceback
                    # traceback.print_exc()
                    final_results = top_candidates[:limit]
            else:
        ...
        logger.exception("search_request_failed", error=str(e), query=q, mode=mode)
...
async def submit_feedback(feedback: FeedbackRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.exception("feedback_submission_failed", error=str(e))
...
async def customs_search(
    ...
    try:
        ...
        logger.exception("customs_search_failed", error=str(e), query=request.query)
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        await indexer.close()
...
async def multimodal_search(
    ...
    try:
        ...
        logger.exception("multimodal_search_failed", error=str(e), query=request.query)
        raise HTTPException(status_code=500, detail=str(e))
