...
async def add_api_key(provider_id: str, request: AddKeyRequest):
    ...
    if request.test:
        test_result = await test_api_key(provider_id=provider_id, api_key=request.api_key)
...
async def get_llm_full_status():
    ...
    async def check_provider(provider_id: str) -> dict:
        ...
        return {
            "id": provider_id,
            "name": get_provider_description(provider_id).split(" - ")[0]
            if " - " in get_provider_description(provider_id)
...
async def test_api_key(provider_id: str, api_key: str, model: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        response = await llm_service.generate(
            prompt="Say 'test successful' and nothing else", provider=provider_id, max_tokens=10
        )
