...
- Порт UI: **3030** (з’єднання через `http://192.168.0.200:3030`).

---
### Примітка про mock‑дані
*Зараз бекенд використовує mock‑заглушки (Neo4jMock, RedisMock, QdrantMock, KafkaMock, MinIOMock).*
Для реального продакшн‑розгортання треба замінити їх на **реальні підключення** (судно iMac або хмарне середовище).
