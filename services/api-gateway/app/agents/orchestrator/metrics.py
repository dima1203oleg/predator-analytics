...
LLM_ROUTER_REQUESTS = Counter(
    'predator_llm_router_requests_total',
    'LLM router requests',
    ['provider', 'status']
)
...
SELF_IMPROVEMENT_PERFORMANCE = Gauge(
    'predator_self_improvement_score',
    'Current self-improvement performance score',
    ['metric']
)
...
ORCHESTRATOR_INFO = Info(
    ...
    'SuperIntelligence Orchestrator information'
)
...
def record_self_improvement(stage: str, success: bool, score: Optional[float] = None):
...
def init_orchestrator_info(version: str, agents: list, llm_providers: list):
    ...
    ORCHESTRATOR_INFO.info({
        'version': version,
        'agents': ','.join(agents),
        'llm_providers': ','.join(llm_providers)
    })
...
class AIMetricsCollector:
    ...
    def initialize(self, orchestrator):
        ...
        try:
            ...
            agents = [agent.value for agent in orchestrator.agents.keys()] if hasattr(orchestrator, 'agents') else []
