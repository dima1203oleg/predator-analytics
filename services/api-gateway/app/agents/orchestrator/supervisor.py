    ...
    async def handle_request(self, user_query: str, mode: str = "auto", request_context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if mode in ["deep", "agentic"]:
            try:
                ...
                return {
                    "query": user_query,
                    "answer": f"⚠️ Agentic Brain Error: {str(e)}",
                    "mode": mode,
        ...
        try:
            ...
            if mode == "council":
                ...
                try:
                    ...
                    logger.error(f"Council failed: {council_error}")
                    # Fallback
                    final_answer = f"Council unavailable ({str(council_error)}). Analyzing via Singleton."
