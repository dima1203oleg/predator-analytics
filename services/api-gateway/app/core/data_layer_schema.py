...
-- =====================================================
CREATE TABLE IF NOT EXISTS raw.etl_jobs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_file VARCHAR(255) NOT NULL,
    state VARCHAR(50) NOT NULL, -- CREATED, UPLOADING, PROCESSING, INDEXING, COMPLETED, FAILED

    -- Progress & Metrics
    progress JSONB DEFAULT '{"percent": 0, "records_total": 0, "records_processed": 0, "records_indexed": 0}',
    timestamps JSONB DEFAULT '{}', -- {created_at, state_entered_at, updated_at}
    errors JSONB DEFAULT '[]', -- List of error objects
...
CREATE INDEX idx_etl_jobs_state ON raw.etl_jobs(state);
CREATE INDEX idx_etl_jobs_created ON raw.etl_jobs(created_at);
