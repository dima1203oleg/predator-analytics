...
async def get_waf_logs(limit: int = 20):
    ...
    if not logs:
        logs = [
            {"id": "waf-1", "ip": "45.132.1.20", "country": "RU", "path": "/api/v1/admin/login", "rule": "geo_block_v4", "action": "BLOCK", "timestamp": datetime.now(timezone.utc).isoformat()},
            {"id": "waf-2", "ip": "102.33.12.1", "country": "CN", "path": "/api/v1/config/dump", "rule": "sqli_pattern_A", "action": "BLOCK", "timestamp": datetime.now(timezone.utc).isoformat()}
        ]
