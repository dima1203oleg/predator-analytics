...
class ETLArbiterAgent:
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                async with get_db_ctx() as sess:
                    ...
                    for job in active_jobs:
                        ...
                        if violations:
                            ...
                            if job.state != derived_state.value:
                ...
                self.logger.error(f"Arbiter Agent error: {e}")
    ...
    def check_invariants(self, job: ETLJob, derived_state: ETLState) -> List[str]:
        ...
        try:
            ...
            self.logger.error(f"OPA check failed: {e}")
            return ["OPA_FAILURE"]
    ...
    async def enforce_invariants(self, job: ETLJob, violations: List[str], derived_state: ETLState):
        ...
        subprocess.run(["python3", "scripts/predatorctl.py", "etl", "force-fail", str(job.id), f"Arbiter Violation: {', '.join(violations)}"])
