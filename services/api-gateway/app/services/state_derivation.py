...
class StateDerivationEngine:
    ...
    def derive_state(self, facts: List[Dict], previous_state: Optional[ETLState]) -> Dict:
        """
        ...
        if previous_state:
            if previous_state in self.TERMINAL_STATES and derived_state != previous_state:
                ...
                violation_msg = f"Terminal state violation: Cannot move from {previous_state}"
    ...
    def _check_heartbeat_violation(self, facts: List[Dict], state: ETLState) -> Optional[str]:
    ...
    def _validate_transition(self, previous_state: Optional[ETLState], derived_state: ETLState) -> bool:
        if previous_state:
            if previous_state in self.TERMINAL_STATES and derived_state != previous_state:
                return False # Terminal state violation
            elif derived_state != previous_state and derived_state not in self.ALLOWED_TRANSITIONS.get(previous_state, set()):
