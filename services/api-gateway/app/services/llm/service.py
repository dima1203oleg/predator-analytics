...
class LLMService:
    ...
    async def _call_cohere(self, prompt, system, config, max_tokens, temperature):
        async with httpx.AsyncClient() as client:
            headers = {
                "Authorization": f"Bearer {random.choice(config['api_keys'])}",
    ...
    async def _call_huggingface(self, prompt, system, config, max_tokens, temperature):
        # Using serverless inference API
        async with httpx.AsyncClient() as client:
            headers = {"Authorization": f"Bearer {random.choice(config['api_keys'])}"}
    ...
    async def _call_gemini(self, prompt, system, config, max_tokens, temperature, format=None):
        async with httpx.AsyncClient() as client:
    ...
    async def _call_openai_compatible(self, prompt, system, config, max_tokens, temperature, provider_name, format=None):
        async with httpx.AsyncClient() as client:
            messages = []
            if system:
            ...
            if "choices" not in data:
                ...
                return LLMResponse(False, "", provider_name, config["model"], error=f"Groq API Error: {data.get('error', {}).get('message', 'No choices in response')}")
