...
from app.models.entities import (
    Source, Dataset, Job, SourceType, DatasetStatus,
    JobType, JobStatus, DataHubStats,
    SourceCreate, SourceUpdate, DatasetCreate, DatasetUpdate,
    SourceResponse, DatasetResponse, JobResponse, UploadWizardResult
)
...
class DataHubService:
    ...
    async def get_source(self, source_id: UUID) -> Optional[SourceResponse]:
        """Get source by ID"""
        async with async_session_maker() as session:
    ...
    async def update_source(self, source_id: UUID, update_data: SourceUpdate) -> Optional[SourceResponse]:
        """Update source"""
        async with async_session_maker() as session:
            # Build update dict
    ...
    async def get_dataset(self, dataset_id: UUID, include_source: bool = True) -> Optional[DatasetResponse]:
        """Get dataset by ID"""
        async with async_session_maker() as session:
    ...
    async def update_dataset(self, dataset_id: UUID, update_data: DatasetUpdate) -> Optional[DatasetResponse]:
        """Update dataset"""
        async with async_session_maker() as session:
            update_dict = update_data.dict(exclude_unset=True)
            if update_dict:
    ...
    async def upload_file_wizard(
        ...
        try:
            ...
            logger.error(f"Upload wizard failed: {e}")
