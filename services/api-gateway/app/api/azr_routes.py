...
async def create_proposal(request: ProposalCreateRequest):
    ...
    try:
        ...
        raise HTTPException(400, f"Invalid category. Must be one of: {[c.value for c in AmendmentCategory]}")
    ...
    if request.rollback_plan:
        from libs.azr.models import RollbackPlan
...
async def submit_approval(
    ...
    from libs.azr.models import CommitteeApproval
    ...
    warnings = []
    if request.committee in ["security", "arbiter"]:
        if not approval.is_unanimous:
...
async def azr_health():
    ...
    return {
        ...
        "opa_connected": True,  # TODO: Implement actual check
        "truth_ledger_accessible": True,  # TODO: Implement actual check
        "timestamp": datetime.utcnow().isoformat()
    }
