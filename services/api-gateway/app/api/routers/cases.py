...
async def list_cases(
    ...
    sector: Optional[str] = Query(None, description="Фільтр за сектором"),
...
async def get_case_stats():
    ...
    try:
        ...
        logger.error(f"Case stats failed: {e}")
...
async def archive_case(case_id: str):
    ...
    try:
        ...
        logger.error(f"Archive case failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def escalate_case(case_id: str):
    ...
    try:
        ...
        logger.error(f"Escalate case failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
