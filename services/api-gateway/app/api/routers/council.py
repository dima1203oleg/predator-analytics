...
_council_instance: Optional[LLMCouncilOrchestrator] = None
...
async def council_strategy_session(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logger.error(f"Strategy session failed: {e}")
...
async def _log_to_mlflow(
    ...
    try:
        ...
        with mlflow.start_run(run_name=f"council_{request_id}"):
            ...
            mlflow.log_metric(
                "deliberation_time",
                result.metadata.get("deliberation_time_seconds", 0)
            )
...
async def _save_council_session(
    ...
    try:
        ...
        logger.error(f"❌ Failed to save council session to DB: {e}")
