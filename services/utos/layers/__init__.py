...
class BaseLayer:
    """Абстрактний шар UTOS. Кожен шар тестування наслідує цей клас."""
    ...
    async def validate(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ Фатальна помилка шару {self.name}: {e}")
            self.checks.append(CheckResult(
                name="layer_crash",
                passed=False,
                message=f"Шар впав: {e}",
                severity="critical",
            ))
        ...
        # Обчислення score шару (0.0–1.0)
        if total == 0:
            layer_score = 0.0
        else:
            layer_score = passed / total
    ...
    async def cmd_check(
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                result = CheckResult(
                    name=name, passed=False,
                    message=f"Exit code {process.returncode}: {error_output[:200]}",
                    severity=severity, latency_ms=latency,
                )
                output = error_output
