# Next Steps — Predator v25.0 Migration & Automation
...
2) **Run Reorganize Script (dry-run first)**
   ```bash
   ./predator_v25/scripts/reorganize_workspace.sh
   # If ok, apply
   ./predator_v25/scripts/reorganize_workspace.sh --apply
   ```
...
5) **Run CI & Add Secret Scanner** — enable `predator_v25/.github/workflows/secret-scan.yml` and other CI jobs
6) **Migrate Helm/ArgoCD** — move existing infra manifests into `predator_v25/infra/helm` and create ArgoCD App-of-Apps
7) **Add a PR template** — require security checklist for secrets and code coverage gates
8) **Create an Issue or PR** — propose transfer of code to `predator_v25` and removal of duplicates
