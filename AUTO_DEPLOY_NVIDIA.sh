...
# ✅ Route через Vite Proxy → NVIDIA Core API (port 8000)
VITE_API_URL=/api/v1
VITE_LITELLM_URL=/v1
VITE_ENABLE_MOCK_API=false
VITE_MODE=remote
VITE_PREDATOR_NODE=SOVEREIGN
VITE_BACKEND_PROXY_TARGET=http://$NVIDIA_IP:8000
EOF
...
sleep 3
if curl -sf --connect-timeout 5 "http://$NVIDIA_IP:8000/health" >/dev/null 2>&1; then
    echo "✅ API ОНЛАЙН: http://$NVIDIA_IP:8000/health"
else
...
echo "🔗 NVIDIA API:  http://$NVIDIA_IP:8000/api/v1"
