...
const TRAINING_CONFIGS: Record<TrainingDomain, { label: string, icon: React.ReactNode, steps: string[] }> = {
    GOV: {
        ...
        steps: [
            "Завантаження реєстрів: 'Єдиний Державний Реєстр Судових Рішень'...",
            "Токенізація законодавчих актів (Кримінальний Кодекс)...",
            "Вирівнювання розпізнавання сутностей (NER) для PEP...",
            "Оптимізація патернів бюрократичної мови..."
        ]
    },
    MED: {
        ...
        steps: [
            "Завантаження протоколів: МКХ-10 та Стандарти МОЗ...",
            "Очищення записів пацієнтів (Видалення PII)...",
            "Навчання векторів кореляції симптом-діагноз...",
            "Тонке налаштування на медичну латину..."
        ]
    },
    BIZ: {
        ...
        steps: [
            "Парсинг логів транзакцій SWIFT...",
            "Аналіз індикаторів волатильності ринку...",
            "Навчання патернів виявлення шахрайства (Anomaly)...",
            "Вирівнювання зі звітністю НБУ..."
        ]
    },
    SCI: {
        ...
        icon: <Leaf size={14} />,
        steps: [
            "Обробка телеметрії супутників (Sentinel-2)...",
            "Калібрування моделей атмосферної дисперсії...",
            "Завантаження потоків гідрологічних сенсорів...",
            "Оптимізація геопросторового мислення..."
        ]
    }
};
...
const MOCK_DSPY_MODULES: DSPyOptimization[] = [
    { id: 'dspy-1', moduleName: 'DeepScan_Reasoning', targetMetric: 'LogicalConsistency', startingScore: 65.4, currentScore: 88.2, iterations: 14, status: 'OPTIMIZING', bestPromptSnippet: '...розбийте аномалію на часові кластери перед класифікацією...', lastImprovement: '+2.1% (ChainOfThought)' },
    { id: 'dspy-2', moduleName: 'SQL_Generator_v2', targetMetric: 'ExecutionSuccess', startingScore: 72.0, currentScore: 94.5, iterations: 42, status: 'CONVERGED', bestPromptSnippet: '...використовуйте явні JOIN по колонках ЄДРПОУ та фільтруйте за статусом active...', lastImprovement: '+0.5% (FewShot)' },
    { id: 'dspy-3', moduleName: 'OSINT_FactChecker', targetMetric: 'HallucinationRate', startingScore: 40.2, currentScore: 76.8, iterations: 8, status: 'OPTIMIZING', bestPromptSnippet: '...перевірте по списку надійних доменів перед цитуванням...', lastImprovement: '+4.3% (BootstrapFewShot)' },
];
...
const LLMView: React.FC = () => {
    ...
    const handleSendMessage = () => {
        ...
        setTimeout(() => {
            ...
            let tokens = 0;
            const fullResponse = "На основі наданих параметрів, суб'єкт 'ТОВ МегаБуд' демонструє показник ризику 0.85 щодо ухилення від сплати податків. Виявлено циклічні транзакції з 'Offshore Ltd' (Кіпр). Рекомендована дія: Глибинний Скан (Deep Scan).";
    ...
    const handleStartTraining = () => {
        ...
        setProgress(0);
        setTrainingLogs([`[INIT] Запуск LoRA Fine-Tuning для ${trainingDomain}...`]);
        setTrainingMetrics([{ step: 0, loss: 2.5, accuracy: 0.1 }]);
        ...
        const totalIterations = 50; // Granular updates for chart
        let currentIter = 0;
        let stepIndex = 0;
        ...
        const interval = setInterval(() => {
            ...
            // Update Charts (Simulate Convergence)
            setTrainingMetrics(prev => {
                const last = prev[prev.length - 1];
                const newLoss = Math.max(0.1, last.loss * 0.95 + (Math.random() * 0.1 - 0.05));
                const newAcc = Math.min(0.99, last.accuracy + (Math.random() * 0.03));
                return [...prev, { step: currentIter, loss: newLoss, accuracy: newAcc }];
            });

            // Update Logs based on "Milestones"
            if (currentIter % 12 === 0 && stepIndex < steps.length) {
                const step = steps[stepIndex];
                setTrainingLogs(prev => [...prev, `[КРОК ${stepIndex + 1}] ${step}`]);
                stepIndex++;
            }

            setProgress((currentIter / totalIterations) * 100);

            if (currentIter >= totalIterations) {
                clearInterval(interval);
                setTrainingStatus('COMPLETED');
                setTrainingLogs(prev => [...prev, "[УСПІХ] Ваги моделі оновлено та завантажено (Hot-Swap)."]);
    ...
    const renderInference = () => (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-300">
            <div className="lg:col-span-2 flex flex-col gap-4 h-[600px]">
                <TacticalCard title={`Сесія Чату: ${activeModel}`} className="flex-1 flex flex-col relative overflow-hidden panel-3d">
                    {/* Chat Area */}
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-slate-950/50">
                        {chatHistory.length === 0 && (
                            <div className="flex flex-col items-center justify-center h-full text-slate-600 gap-4">
                                ...
                                <p>AI Готовий. Контекстне Вікно: 32k. Temperature: 0.7</p>
                            </div>
                        )}
                    ...
                    <div className="p-4 bg-slate-900 border-t border-slate-800">
                        ...
                        <div className="flex justify-between items-center mt-2 text-[10px] text-slate-500 font-mono">
                            <span>Швидкість: {genMetrics.tps} tok/s</span>
                            <span>Затримка: {genMetrics.ttft} ms</span>
                            <span>Токени: {genMetrics.totalTokens}</span>
                        </div>
                    </div>
                </TacticalCard>
            </div>
            ...
            <div className="space-y-6">
                <TacticalCard title="Параметри Моделі" className="panel-3d">
                    <div className="space-y-4">
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Системний Промпт</label>
                            <textarea 
                                value={systemPrompt}
                ...
                <TacticalCard title="LLM Роутер & Ротація" className="panel-3d">
    ...
    const renderTraining = () => (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-300">
            <TacticalCard title="LoRA Консоль Тонкого Налаштування" className="panel-3d">
                <div className="space-y-6">
                    ...
                    <div className="p-4 bg-slate-950 border border-slate-800 rounded">
                        ...
                        {trainingMetrics.length > 0 ? (
                            ...
                            <div className="h-40 flex items-center justify-center bg-slate-900/50 rounded border border-dashed border-slate-800 text-xs text-slate-500 mb-3">
                                Очікування даних графіку...
                            </div>
                        )}

                        <div className="h-32 overflow-y-auto custom-scrollbar bg-black/50 p-2 rounded border border-slate-800 font-mono text-[10px] space-y-1">
                            {trainingLogs.length === 0 ? (
                                <div className="text-slate-600 italic">Готовий до запуску тренування...</div>
                            ) : (
                                trainingLogs.map((log, i) => (
                                    <div key={i} className="text-slate-300 flex gap-2 animate-in slide-in-from-left-2">
                    ...
                    <div className="flex justify-end gap-3">
                        <button className="px-4 py-2 rounded text-xs font-bold text-slate-300 btn-3d">Гіперпараметри</button>
                        <button 
                        ...
                        >
                            ...
                            {trainingStatus === 'TRAINING' ? 'Навчання...' : 'Розпочати'}
                        </button>
                    </div>
                </div>
            </TacticalCard>

            <TacticalCard title="AutoML Експерименти" className="panel-3d">
    ...
    const renderDSPy = () => (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-300">
            <TacticalCard title="DSPy Оптимізатор Промптів (Автопілот)" className="panel-3d" action={
                <button 
                ...
                >
                    ...
                    {dspyOptimizing ? 'Оптимізація...' : 'Запустити Оптимізатор'}
                </button>
            }>
                        ...
                        <div key={mod.id} className="p-3 bg-slate-900 border border-slate-800 rounded group hover:border-purple-500/30 transition-colors panel-3d">
                            ...
                            <div className="mt-2 bg-black/40 p-2 rounded border border-slate-800/50">
                                <div className="flex items-center gap-2 mb-1 text-[9px] text-slate-500 uppercase font-bold">
                                    <GitCompare size={10} /> Оптимізований Фрагмент Інструкції
                                </div>
            ...
            <TacticalCard title="Логи Оптимізації" className="panel-3d">
                <div className="h-[500px] overflow-y-auto custom-scrollbar font-mono text-[10px] space-y-2 p-2">
                    {dspyOptimizing && (
                        <>
                            <div className="text-purple-400 animate-pulse">>> DSPy BootstrapFewShot: Запуск Ітерації {dspyData.length}...</div>
                            <div className="text-slate-400">[Compiler] Генерую 5 кандидатів промптів для `DeepScan_Reasoning`...</div>
                            <div className="text-slate-400">[Evaluator] Запуск 20 тест-кейсів проти метрики `TruthProtocol`...</div>
                            <div className="text-slate-300 flex gap-2">
                                <span className="text-yellow-500">[Candidate 1]</span> Score: 86.5% - Відхилено
                            </div>
                            <div className="text-slate-300 flex gap-2">
                                <span className="text-success-500">[Candidate 2]</span> Score: 89.1% - Прийнято (Новий Рекорд)
                            </div>
                            <div className="text-slate-500 italic pl-4">Reasoning: "Кандидат 2 явно запитує часову кластеризацію, покращуючи логічний скор."</div>
                        </>
                    )}
                    {!dspyOptimizing && <div className="text-slate-600 text-center mt-20">Оптимізатор в режимі очікування. Натисніть Старт для запуску циклу самовдосконалення.</div>}
                </div>
            </TacticalCard>
        </div>
    );
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20 w-full max-w-[1600px] mx-auto">
            <ViewHeader 
                title="Нейронне Ядро (LLM & Neural Core)"
                icon={<BrainCircuit size={20} className="icon-3d-purple"/>}
                breadcrumbs={['ІНТЕЛЕКТ', 'НЕЙРОННИЙ ДВИГУН']}
                stats={[
                    ...
                    { label: 'DSPy Оптимізатор', value: dspyOptimizing ? 'ПРАЦЮЄ' : 'ОЧІКУВАННЯ', icon: <Sparkles size={14}/>, color: dspyOptimizing ? 'purple' : 'default', animate: dspyOptimizing },
                ]}
            />

            {/* Tabs */}
            <div className="flex border-b border-slate-800 bg-slate-900/30 rounded-t overflow-x-auto scrollbar-hide">
                <button 
                ...
                >
                    <MessageSquare size={16} /> Чат (Inference)
                </button>
                <button 
                ...
                >
                    <Sparkles size={16} /> DSPy Оптимізатор
                </button>
                <button 
