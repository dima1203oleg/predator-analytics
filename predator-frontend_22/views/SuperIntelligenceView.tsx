...
import { 
  ...
  Maximize2, Minimize2, Cpu, Microchip, Lock, EyeOff
} from 'lucide-react';
                ...
                <ViewHeader 
                    ...
                    actions={
                        <div className="flex gap-2">
                            <button 
                                onClick={toggleLoop}
                                className={`px-4 py-2 rounded text-xs font-bold flex items-center gap-2 border transition-all btn-3d ${
                                    isActive 
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-slate-950 via-slate-900/80 to-transparent pointer-events-none">
                            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide justify-center pointer-events-auto">
                                {activeAgents.map(agent => (
                                    <div key={agent.id} className={`flex flex-col items-center gap-2 p-2 rounded-lg border backdrop-blur-sm transition-all ${
                                        agent.status !== 'IDLE' 
                                        ...
                                        <div className={`p-2 rounded-full ${
                                            agent.role === 'SCANNER' ? 'bg-blue-500/20 text-blue-400' :
                            ...
                            <div className="flex border-b border-slate-800 bg-slate-900/50 overflow-x-auto scrollbar-hide">
                                {[
                                    ...
                                    <button 
                                        ...
                                        className={`flex-1 min-w-[80px] py-3 text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors border-b-2 ${
                                            rightTab === tab.id 
                                                    ...
                                                    if (log.type === 'DEBATE') {
                                                        ...
                                                        return (
                                                            <div key={log.id} className={`flex gap-3 mb-3 animate-in slide-in-from-bottom-2 ${isRight ? 'flex-row-reverse' : ''}`}>
                                                                <div className={`w-8 h-8 rounded border flex items-center justify-center shrink-0 font-bold text-xs shadow-md ${
                                                                    isRight ? 'bg-blue-900/20 border-blue-500 text-blue-400' : 'bg-purple-900/20 border-purple-500 text-purple-400'
                                                                }`}>
                                                                    {model?.avatar || log.source[0]}
                                                                </div>
                                                                <div className={`max-w-[80%] p-2.5 rounded-lg border text-xs leading-relaxed shadow-sm ${
                                                                    isRight ? 'bg-slate-800/50 border-slate-700 text-slate-200 rounded-tr-none' : 'bg-slate-900/50 border-slate-800 text-slate-300 rounded-tl-none'
                                                                }`}>
                                    ...
                                    <div className="absolute inset-0 overflow-hidden">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                            <div className="text-center p-6 bg-slate-900/80 border border-slate-800 rounded-xl backdrop-blur animate-pulse">
                                                <Microchip size={48} className="text-blue-500 mx-auto mb-2" />
                                                    ...
                                                    <div className="flex justify-between items-start mb-2 pl-3">
                                                        <div className="flex items-center gap-2">
                                                            <div className={`p-1.5 rounded ${
                                                                art.type === 'CODE' ? 'bg-blue-900/20 text-blue-400' : 
                                    ...
                                    <div className="absolute inset-0 p-4 overflow-y-auto custom-scrollbar space-y-3">
                                        ...
                                        {agentGenomes.length === 0 ? (
                                            ...
                                            agentGenomes.map(agent => (
                                                <div key={agent.agentId} className={`p-3 bg-slate-900 border rounded flex flex-col gap-2 transition-all ${
                                                    agent.evolutionStatus === 'EVOLVING' ? 'border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.2)]' : 'border-slate-800'
                                                }`}>
                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        <div className="text-right">
                                                            <div className={`text-[9px] font-bold px-2 py-0.5 rounded border uppercase ${
                                                                agent.evolutionStatus === 'EVOLVING' ? 'bg-purple-900/20 text-purple-400 border-purple-900/50' :
