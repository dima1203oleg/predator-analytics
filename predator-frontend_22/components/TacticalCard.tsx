...
export const TacticalCard: React.FC<TacticalCardProps> = ({ 
  ...
  return (
    <div className={`flex flex-col transition-all duration-500 group ${getGlowClass()} ${baseClass} ${className} relative overflow-hidden`}>
      ...
      <div className="absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.05)_50%),linear-gradient(90deg,rgba(255,0,0,0.03),rgba(0,255,0,0.01),rgba(0,0,255,0.03))] z-0 pointer-events-none bg-[size:100%_4px,6px_100%] opacity-20"></div>

      {title && (
        <div className="px-5 py-4 border-b border-slate-800/50 flex justify-between items-center bg-slate-950/30 rounded-t-xl relative z-10">
      ...
      <div className={`${noPadding ? '' : 'p-5'} flex-1 relative z-10`}>
        {children}
      </div>
    </div>
  );
