...
export const Relations: React.FC = () => {
  ...
  // Mock nodes
  const nodes = [
    { id: 1, x: 50, y: 50, label: premiumLocales.entityGraph.mockNodes.companyA, type: 'company' },
    { id: 2, x: 30, y: 30, label: premiumLocales.entityGraph.mockNodes.directorB, type: 'person' },
    { id: 3, x: 70, y: 60, label: premiumLocales.entityGraph.mockNodes.offshoreC, type: 'entity' },
    { id: 4, x: 40, y: 70, label: premiumLocales.entityGraph.mockNodes.bankD, type: 'bank' },
  ];
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h1 className="text-2xl font-bold text-white">{premiumLocales.entityGraph.title}</h1>
          <p className="text-slate-400 text-sm">{premiumLocales.entityGraph.toolbar.subtitle}</p>
        </div>
      ...
      <div className="flex-1 bg-slate-900 border border-slate-700 rounded-xl relative ">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
           <div className="relative w-full h-full">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
                 ...
                 <span className="mt-2 text-white font-bold bg-slate-900/80 px-2 py-0.5 rounded">{premiumLocales.entityGraph.mockNodes.companyA}</span>
              </div>
              ...
              <div className="absolute top-[30%] left-[30%] flex flex-col items-center">
                 ...
                 <div className="mt-2 text-xs text-slate-300 bg-slate-900/80 px-2 py-0.5 rounded">
                    {isEnabled ? premiumLocales.entityGraph.mockNodes.directorB : '******'}
                 </div>
              </div>
              ...
              <div className="absolute top-[60%] left-[70%] flex flex-col items-center">
                 ...
                 <div className="mt-2 text-xs text-slate-300 bg-slate-900/80 px-2 py-0.5 rounded">
                    {isEnabled ? premiumLocales.entityGraph.mockNodes.offshoreC : '******'}
                 </div>
              </div>
           </div>
        </div>
        ...
        {!isEnabled && (
           <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-amber-500/10 border border-amber-500/20 px-4 py-2 rounded-lg text-amber-500 text-sm font-bold flex items-center gap-2">
              {premiumLocales.entityGraph.toolbar.hiddenDataWarning}
           </div>
        )}
      </div>
    </div>
