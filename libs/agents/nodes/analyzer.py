...
from ..state import AgentState, AnalysisResult, CaseTemplate, QueryResult
...
async def analyzer_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        fusion_results = {
            ...
            "total_records": sum(
                len(r.get("results", [])) for r in query_results.values()
            ),
        ...
        logger.error(f"E2E Analyzer failed: {e}")
        return {
            "error": f"Помилка аналізу: {str(e)}",
            "stream_updates": updates + [f"❌ Помилка: {str(e)}"]
        }
