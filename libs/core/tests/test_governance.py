...
class TestGovernance(unittest.TestCase):
    def test_forbidden_commands(self):
        ...
        for cmd in bad_commands:
            ...
            self.assertFalse(res["approved"], f"Command '{cmd}' should be blocked!")
    ...
    def test_safe_commands(self):
        ...
        for cmd in good_commands:
            res = OperationalPolicy.validate_command(cmd)
            self.assertTrue(res["approved"], f"Command '{cmd}' should be allowed!")
    ...
    def test_production_constraints(self):
        ...
        self.assertTrue(res_rnd["approved"])

        # Blocked in PRODUCTION
        res_prod = OperationalPolicy.validate_command(cmd, stage=SecurityStage.PRODUCTION)
        self.assertFalse(res_prod["approved"], "Direct kubectl apply should be blocked in PROD!")

    def test_tech_rationalization(self):
        """Test technology white/black listing."""
        self.assertFalse(OperationalPolicy.check_technology(["php"])["approved"])
        self.assertTrue(OperationalPolicy.check_technology(["python", "postgres"])["approved"])
