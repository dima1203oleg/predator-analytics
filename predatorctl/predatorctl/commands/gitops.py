...
@app.command()
def sync(
    application: Optional[str] = typer.Option(None, "--application", help="Specific application to sync")
):
    """
    Synchronize local state with GitOps source.
    """
    target = application if application else "all applications"
    console.print(f"Syncing {target}...")
    console.print("[green]Synced[/green]")

@app.command()
def diff(
    application: Optional[str] = typer.Option(None, "--application", help="Specific application to diff")
):
    """
...
@app.command()
def history(
    application: Optional[str] = typer.Option(None, "--application", help="Specific application"),
