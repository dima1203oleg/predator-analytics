...
async def test_high_concurrency_load():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        failure = [r for r in responses if not isinstance(r, httpx.Response) or r.status_code != 200]
        
        print(f"Load Test: {len(success)} success, {len(failure)} failed in {end_time - start_time:.2f}s")
