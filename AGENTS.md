...
- **Код Backend**: ТІЛЬКИ Python 3.12. Використання інших версій категорично заборонено.
- **Код Frontend**: TypeScript/React (frontend)
- **Коментарі та документація**: ВИКЛЮЧНО українською мовою.
- **Git commits**: формат: `feat|fix|chore|docs(scope): опис` (тільки українською)
- **UI тексти та локалізація**: ПОВНА УКРАЇНІЗАЦІЯ веб-інтерфейсу. 100% текстів, кнопок, алертів, підказок та логів у UI мають бути виключно українською. Англійська в UI = критична помилка (HR-04). Жодних винятків.
