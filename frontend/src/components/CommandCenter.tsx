...
const CommandCenter: React.FC<CommandCenterProps> = ({ isOpen, onClose, onLock, onLogout, onReboot, onNavigate }) => {
  ...
  const [logs, setLogs] = useState<CommandLog[]>([
    { 
      id: 'init', 
      command: 'system info', 
      output: <span className="text-slate-400">Predator Analytics v18.6 [Truth-Only Edition]<br/>Kernel: Linux 5.15.0-generic (x86_64)<br/>Node: k3s-master-01 [ONLINE]<br/>Locale: uk_UA.UTF-8<br/>Use 'go [view]' to navigate.</span>, 
  ...
  const processCommand = (cmd: string): React.ReactNode => {
    ...
    if (cmd.startsWith('go ') || cmd.startsWith('open ')) {
        ...
        if (onNavigate) {
            switch (target) {
                case 'dashboard': onNavigate(TabView.DASHBOARD); return 'Redirecting to Dashboard...';
                case 'god': case 'super': onNavigate(TabView.SUPER_INTELLIGENCE); return 'Entering God Mode...';
                case 'analytics': case 'scan': onNavigate(TabView.ANALYTICS); return 'Opening Deep Scan...';
                case 'devops': case 'infra': onNavigate(TabView.DEVOPS); return 'Opening Engineering Hub...';
                case 'brain': case 'council': onNavigate(TabView.SYSTEM_BRAIN); return 'Summoning Council...';
                case 'nas': onNavigate(TabView.NAS); return 'Opening NAS Arena...';
                case 'security': case 'defcon': onNavigate(TabView.SECURITY); return 'Opening Security Center...';
                case 'chat': case 'user': onNavigate(TabView.USER_PORTAL); return 'Switching to Executive Mode...';
                default: return <span className="text-yellow-500">Unknown view. Try: dashboard, god, analytics, devops, brain</span>;
            }
        }
    }
    ...
    switch (cmd) {
      case 'help': return <span className="text-primary-400">
          ...
          - go [view]: Navigate (e.g., 'go god', 'go brain')<br/>
          - status: System health check<br/>
          - scan: Quick threat scan<br/>
          - lock: Lock session<br/>
          - matrix: Toggle visual effect<br/>
  ...
  return (
    ...
    >
      <div 
          className={`w-full h-[100dvh] md:h-auto md:max-w-4xl border rounded-xl shadow-2xl overflow-hidden flex flex-col md:max-h-[700px] mt-0 md:mt-16 transition-all duration-300 panel-3d ${matrixMode ? 'bg-black border-green-500/50 shadow-[0_0_50px_rgba(0,255,0,0.2)]' : 'bg-slate-900 border-slate-700'}`}
      ...
      >
        ...
        <div className={`p-2 border-t ${matrixMode ? 'border-green-900 bg-black' : 'border-slate-800 bg-slate-900'} relative z-10 overflow-x-auto`}>
            <div className="flex gap-2 text-[10px] font-mono">
                <span className="text-slate-500 px-2">Quick Nav:</span>
        ...
        <form onSubmit={handleCommand} className={`p-3 border-t flex items-center gap-2 shrink-0 relative z-10 ${matrixMode ? 'bg-black border-green-900' : 'bg-slate-900 border-slate-800'}`}>
          ...
          <input 
            ...
            placeholder={matrixMode ? "Follow the white rabbit..." : "Введіть команду або 'go [view]'..."}
            autoComplete="off"
          />
        </form>
      </div>
