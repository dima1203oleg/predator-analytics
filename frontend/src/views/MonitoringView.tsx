...
const MonitoringView: React.FC = () => {
    ...
    useEffect(() => {
        setResourceData(prev => {
            // Keep updating resource chart even if not strictly fetching from API, 
            // but ensure component is mounted to prevent leaks if this was an async fetch
                ...
                <div className="space-y-3">
                    {MOCK_SAGAS.map(saga => (
                        <div
                            ...
                            className={`p-3 bg-slate-950 border rounded cursor-pointer transition-all btn-3d ${selectedSaga?.id === saga.id ? 'border-primary-500 shadow-[0_0_10px_rgba(6,182,212,0.2)]' : 'border-slate-800 hover:border-slate-600'
                                }`}
                        >
                            <div className="flex justify-between items-start mb-1">
                                <span className="text-xs font-bold text-slate-200">{saga.name}</span>
                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${saga.status === 'COMPLETED' ? 'bg-success-900/20 text-success-500' :
            ...
            <TacticalCard title="Візуалізація Оркестрації" className="lg:col-span-2 panel-3d">
                {selectedSaga ? (
                        ...
                        <div className="relative pl-8 space-y-6">
                            ...
                            {selectedSaga.steps.map((step, idx) => (
                                <div key={step.id} className="relative flex items-center gap-4 group">
                                    {/* Status Icon */}
                                    <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center bg-slate-950 z-10 icon-3d ${step.status === 'COMPLETED' ? 'border-success-500 text-success-500' :
                                    ...
                                    {/* Step Card */}
                                    <div className={`flex-1 p-3 rounded border flex justify-between items-center btn-3d ${step.status === 'FAILED' ? 'bg-red-900/10 border-red-900/50' :
                                        ...
                                        <div>
                                            ...
                                            <div className="text-[10px] text-slate-400 font-mono mt-1">
                                                Дія: <span className="text-slate-200">{step.action}</span>
                                            </div>
                                            {step.status === 'COMPENSATED' && (
                                                <div className="text-[10px] text-orange-400 font-mono mt-1 flex items-center gap-1">
                                                    <RotateCcw size={10} /> Компенсація: {step.compensatingAction}
                                                </div>
                                            )}
                                        </div>
                    ...
                    <div className="h-full flex items-center justify-center text-slate-500">
                        Оберіть транзакцію для перегляду трейсу.
                    </div>
                )}
            </TacticalCard>
        </div>
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-24 w-full max-w-[1600px] mx-auto">
            ...
            <ViewHeader
                ...
                breadcrumbs={['СИСТЕМА', 'МОНІТОРИНГ']}
                stats={[
                    { label: 'Prometheus', value: 'ONLINE', icon: <Eye size={14} />, color: 'success' },
                    { label: 'Saga Tx', value: 'ACTIVE', icon: <Layers size={14} />, color: 'primary' },
                    { label: 'Сповіщення', value: String(mockAlerts.length), icon: <Activity size={14} />, color: mockAlerts.some(a => a.severity === 'critical') ? 'danger' : 'warning' },
                ]}
            />
            ...
            <div className="flex border-b border-slate-800 mb-6 bg-slate-950/30 rounded-t overflow-x-auto scrollbar-hide">
                ...
                >
                    <Layers size={16} /> Saga Транзакції
                </button>
            </div>
                            ...
                            <div className="h-[250px] w-full">
                                <ReactECharts
                                    option={{
                                        ...
                                        legend: { data: ['CPU Usage (%)', 'Memory Usage (GB)'], textStyle: { color: '#94a3b8' }, bottom: 0 },
