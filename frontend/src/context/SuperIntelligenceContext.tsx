...
const INITIAL_LOGS: SIContextLog[] = [
    { id: 'init-0', timestamp: '09:59:55', type: 'INFO', source: 'KERNEL', message: 'Booting Singularity Core v20.0...' },
...
export const SuperIntelligenceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [logs, setLogs] = useState<SIContextLog[]>(INITIAL_LOGS);
    ...
    const [nasDiff, setNasDiff] = useState(INITIAL_NAS_BOOT_CODE); // Pre-populate!
    ...
    const [ragArtifacts, setRagArtifacts] = useState<RAGArtifact[]>(INITIAL_ARTIFACTS);
    ...
    const runLoop = async () => {
        ...
        for (const turn of nextScenario.debate) {
            ...
            addLog('DEBATE', turn.id.toUpperCase(), turn.msg); // Log as distinct DEBATE type
    ...
    useEffect(() => {
        if (isActive) {
            ...
            setStage('IDLE');
            // Do not clear nasDiff on stop, so it remains visible
            setActiveAgents(prev => prev.map(a => ({ ...a, status: 'IDLE' })));
            setBrainNodes(prev => prev.map(n => ({ ...n, status: 'IDLE' })));
        }
        return () => {
