...
const Layout: React.FC<LayoutProps> = ({
  ...
  const [metrics, setMetrics] = useState({ cpu: 34, ram: 68, temp: 42, rps: 14 });
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics({
        cpu: Math.floor(30 + Math.random() * 20),
        ram: Math.floor(60 + Math.random() * 15),
        temp: Math.floor(40 + Math.random() * 10),
        rps: Math.floor(10 + Math.random() * 10),
      });
    }, 2000);
  ...
  return (
    <div className={`
      flex h-[100dvh] bg-slate-950 overflow-hidden font-sans text-slate-200
