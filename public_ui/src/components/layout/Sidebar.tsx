...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, isCompact = false }) => {
  ...
  const getNavItems = () => {
    ...
    items = [...items, ...NAVIGATION_CONFIG.client];

    // Premium section (visible if Premium or Admin)
    if (role === UserRole.CLIENT_PREMIUM || role === UserRole.ADMIN) {
        items = [...items, ...NAVIGATION_CONFIG.premium];
    }

    // Admin section (visible ONLY if Admin)
    // NOTE: Even if Admin technically has access to basic stuff,
    // we might want to segregate. But here we append it for unified view or replace?
    // According to TZ: "All users work inside one application".
    // Let's append Admin items at the bottom if Admin.
    if (role === UserRole.ADMIN) {
      items = [...items, ...NAVIGATION_CONFIG.admin];
    }

    // Filter out items not allowed for current role (double check)
    return items.filter(item => item.roles.includes(role) || (role === UserRole.ADMIN && item.path.startsWith('/admin')));
  };
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!isCompact && (
              <div className="min-w-0">
                <h1 className="font-black text-xl tracking-tight text-white truncate">PREDATOR</h1>
