...
echo "🌌 Надсилання команд на iMac (192.168.0.114)..."
echo "⚠️  Введіть пароль '1204' якщо запитає."

# Спершу чистимо завислі контейнери з правильним PATH
sshpass -p '1204' ssh dmytrokizima@192.168.0.114 "export PATH=\$PATH:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin && docker ps -a | grep k3d-predator-full-stack | awk '{print \$1}' | xargs docker rm -f 2>/dev/null || true"

# Потім запускаємо повний бутстрап
cat deploy/scripts/deploy_imac_full_stack.sh | sshpass -p '1204' ssh dmytrokizima@192.168.0.114 "export PATH=\$PATH:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin && cat > ~/bootstrap.sh && chmod +x ~/bootstrap.sh && bash ~/bootstrap.sh"
...
echo "🔗 iMac API: http://192.168.0.114:8000/api/v1"
