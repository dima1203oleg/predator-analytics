        ...
        <div>
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] mb-3">
                <div className="bg-slate-950/50 p-1.5 rounded border border-slate-800/50 text-center">
                    <div className="text-slate-500">Запити/Хв</div>
                    <div className="font-mono text-slate-200">{connector.rpm}</div>
                </div>
                <div className="bg-slate-950/50 p-1.5 rounded border border-slate-800/50 text-center">
                    <div className="text-slate-500">Латенсі</div>
                    <div className="font-mono text-slate-200">{connector.latency}мс</div>
                </div>
            </div>
        </div>
...
const ParsersView: React.FC = () => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-safe w-full max-w-[1600px] mx-auto">
            <ViewHeader
                title="Двигун Завантаження Даних (ETL)"
                icon={<Workflow size={20} className="icon-3d-amber" />}
                breadcrumbs={['ДАНІ', 'ПАЙПЛАЙНИ ЗАВАНТАЖЕННЯ']}
                stats={[
                    { label: 'Активні Джерела', value: String(connectors.length), icon: <Globe size={14} />, color: 'primary' },
                    { label: 'Пропускна Здатність', value: '14.2k/хв', icon: <Zap size={14} />, color: 'warning', animate: true },
                    { label: 'Якість Даних', value: '99.9%', icon: <Shield size={14} />, color: 'success' },
                ]}
            />
