...
const AgentsView: React.FC = () => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-20 w-full max-w-[1600px] mx-auto">
            <ViewHeader
                title="Моніторинг Флоту Агентів (MAS)"
                icon={<Server size={20} className="icon-3d-blue" />}
                breadcrumbs={['КОМАНДУВАННЯ', 'МЕНЕДЖЕР ФЛОТУ']}
                stats={[
                    { label: 'Активні Поди', value: String(agents.length), icon: <Bot size={14} className="icon-3d-purple" />, color: 'primary' },
                    { label: 'Навантаження', value: '34%', icon: <Cpu size={14} className="icon-3d-amber" />, color: 'success' },
                    { label: 'Шина Даних', value: 'ОНЛАЙН', icon: <Network size={14} className="icon-3d-green" />, color: 'success' },
                ]}
            />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {agents.map(agent => (
                            <div
                                ...
                                className={`p-4 rounded-lg border cursor-pointer transition-all btn-3d panel-3d ${selectedAgentId === agent.id
                            ...
                            >
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex items-center gap-3">
                                        <div className={`p-2 rounded-lg icon-3d ${agent.status === 'WORKING' ? 'bg-yellow-900/20 text-yellow-400 animate-pulse' :
                                    ...
                                    <div className={`px-2 py-0.5 rounded text-[9px] font-bold border ${agent.status === 'WORKING' ? 'bg-yellow-900/20 text-yellow-500 border-yellow-900/50' :
                ...
                <div className="space-y-6">
                    <TacticalCard title="Телеметрія та Здоров'я" className="panel-3d">
                        {selectedAgent ? (
                            <div className="space-y-6">
                                <div className="flex items-center justify-between p-3 bg-slate-950/50 border border-slate-800 rounded panel-3d">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-xs font-bold text-slate-300">Здоров'я Pod</div>
                                            <div className="text-[10px] text-slate-500">Аптайм: 14д 2г</div>
                                        </div>
                                    </div>
                                ...
                                <div className="h-[200px] w-full">
                                    <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-2">Використання Ресурсів (Наживо)</h4>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-2 bg-slate-950/80 rounded border border-slate-800 text-center btn-3d">
                                        ...
                                        <div className="text-[10px] text-slate-500">Ліміт CPU</div>
                                    ...
                                    <div className="p-2 bg-slate-950/80 rounded border border-slate-800 text-center btn-3d">
                                        ...
                                        <div className="text-[10px] text-slate-500">Ліміт Пам'яті</div>
                    ...
                    <TacticalCard title="Системні Сповіщення" className="panel-3d">
                        <div className="space-y-2">
                            <div className="p-2 bg-yellow-900/10 border border-yellow-900/30 rounded flex items-start gap-2 text-[10px] text-yellow-500 btn-3d">
                                ...
                                <span>Високий тиск пам'яті на node-02 (Worker). Авто-масштабування вимкнено.</span>
                            </div>
                            <div className="p-2 bg-slate-950/50 border border-slate-800 rounded flex items-start gap-2 text-[10px] text-slate-400 btn-3d">
                                ...
                                <span>Регулярна перевірка здоров'я Оркестратора пройдена.</span>
                            </div>
                        </div>
                    </TacticalCard>
                </div>
