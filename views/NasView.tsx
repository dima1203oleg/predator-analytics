...
const NasView: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Transform models for scatter chart
        const data = models.filter(m => m.status === 'COMPLETED').map(m => ({
            x: m.metrics.latency,
            y: m.metrics.accuracy * 100,
            z: m.metrics.params / 1000000, // Size in M
            name: m.architecture,
        ...
        const interval = setInterval(() => {
            ...
            setTournaments(prev => prev.map(t => {
                if (t.status === 'RUNNING') {
                    return { ...t, candidatesCount: t.candidatesCount + 1, bestScore: Math.min(0.99, t.bestScore + 0.001) };
                }
                return t;
            }));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-300">
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {tournaments.map(t => (
                        <div key={t.id} className="bg-slate-900 border border-slate-800 p-4 rounded-lg relative overflow-hidden group panel-3d">
                            {t.status === 'RUNNING' && <div className="absolute top-0 left-0 w-full h-0.5 bg-green-500 animate-progress"></div>}
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded ${t.status === 'RUNNING' ? 'bg-green-900/20 text-green-500 animate-pulse' : 'bg-slate-800 text-slate-500'}`}>
                                        <Trophy size={20} />
                                    </div>
                                    <div>
