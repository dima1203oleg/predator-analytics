                    ...
                    >
                        ...
                        <div className="flex gap-2 mb-4 relative z-10">
                            {node.features?.map(feat => (
                                <span key={feat} className={`text-[9px] px-1.5 py-0.5 rounded border font-bold flex items-center gap-1 btn-3d ${feat === 'SGX' ? 'bg-orange-900/20 text-orange-400 border-orange-900/50' :
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 relative z-10">
                            {node.pods.map(pod => (
                                ...
                                >
                                    {/* Status Indicator */}
                                    <div className={`absolute top-2 right-2 w-2 h-2 rounded-full ${pod.status === 'Running' ? 'bg-success-500 shadow-[0_0_5px_lime]' :
                                            pod.status === 'Pending' ? 'bg-yellow-500 animate-pulse' :
                                                'bg-red-500 animate-ping'
                                        }`}></div>

                                    {/* Icon */}
                                    <div className={`p-2 rounded-full icon-3d ${pod.type === 'db' ? 'bg-blue-900/20 text-blue-400' :
                    ...
                    <div className="space-y-2">
                        ...
                        {localPRs.map((pr) => (
                            ...
                            >
                                ...
                                <div className="text-right">
                                    <div className={`text-[10px] font-bold px-2 py-0.5 rounded mb-1 ${pr.status === 'OPEN' ? 'bg-success-900/20 text-success-500' :
                            ...
                            {testResults.map((test, idx) => (
                                <div key={idx} className="flex justify-between items-center p-2 bg-slate-900 rounded border border-slate-800 btn-3d cursor-default">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold w-16 text-center ${test.status === 'PASSED' ? 'bg-success-900/20 text-success-500 border border-success-900/30' :
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 relative pb-24 w-full max-w-[1600px] mx-auto">
                        ...
                        <div className="flex border-b border-slate-800 bg-slate-900">
                            {['LOGS', 'TERMINAL', 'DESCRIBE'].map((tab) => (
                                <button
                                    ...
                                    className={`flex-1 py-3 text-xs font-bold transition-colors border-b-2 ${consoleTab === tab
            ...
            <ViewHeader
                ...
                breadcrumbs={['СИСТЕМА', 'ІНФРАСТРУКТУРА']}
                stats={[
                    { label: 'K3s Версія', value: 'v1.28.2', icon: <CheckCircle2 size={14} />, color: 'success' },
                    { label: 'Git Репо', value: 'predator-infra', icon: <GitBranch size={14} />, color: 'primary' },
                    { label: 'HPA', value: 'АКТИВНО', icon: <Scale size={14} />, color: 'warning', animate: true }
                ]}
            />
            ...
            <div className="flex border-b border-slate-800 mb-6 bg-slate-950/30 rounded-t overflow-x-auto">
                ...
                >
                    <Server size={16} /> Карта Кластера (SGX)
                </button>
                <button
                ...
                >
                    <GitBranch size={16} /> GitOps Пайплайн
                </button>
                <button
