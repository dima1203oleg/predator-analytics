...
if command -v tox >/dev/null 2>&1; then
  ...
  echo "tox not found; running inside python:3.12-slim"
  docker run --rm -v "$PWD":/workspace -w /workspace -u $(id -u):$(id -g) python:3.12-slim bash -lc "pip install tox && tox -e py312" || true
fi
