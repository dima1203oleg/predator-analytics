...
- Prefer `python:3.12-slim` in your Dockerfiles and build multi-stage images to keep image size small.
- For production runtime, consider `gcr.io/distroless/python3` or `python:3.12-slim` as a safety fallback.
...
## Developer Automation Tools (Added Jan 2026)

### 1. Generate Standard Service Dockerfile
Create a new Python 3.12 compliant Dockerfile for a new service:
```bash
./scripts/generate-dockerfile.sh <service_name>
```

### 2. Local E2E Smoke Tests
Verify backend health and monitoring endpoints locally:
```bash
./scripts/run-e2e.sh
```
...
- `make lint` - Validate code style (Ruff)
