...
tabs_code = """
              ...
              {activeEntity.type === 'person' && (
                  <div className="flex flex-wrap gap-2 border-b border-slate-800 pb-2 mb-4">
                    <button 
              ...
              {/* Main Content conditionally wrapped for Person */}
              {(activeEntity.type !== 'person' || activePersonTab === 'general') && (
                <div className="space-y-5">
"""
lines.insert(description_idx, tabs_code)

for i, line in enumerate(lines):
    if "{/* OSINT Relationship Graph (Section 13) */}" in line:
        graph_idx = i
        break
...
"""
lines.insert(graph_idx, other_tabs_code)

with open(filepath, 'w') as f:
    f.writelines(lines)
print("Dossier UI updated with Person tabs.")
