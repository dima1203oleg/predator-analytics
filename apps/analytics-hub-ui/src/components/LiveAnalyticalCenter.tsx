 ...
 * NEXUS - Living AI Intelligence Core & Analytical Universe
 */
...
export default function LiveAnalyticalCenter({
  ...
  const [lastSpokenText, setLastSpokenText] = useState(
    "Доброго дня, Дмитре. Проаналізовано 5 347 нових подій.",
  );
  ...
  const [coreState, setCoreState] = useState<
    "idle" | "learning" | "optimization" | "inference" | "validation"
  >("idle");
  const [workMode, setWorkMode] = useState<
    "explorer" | "business" | "expert" | "presentation" | "command-center"
  >("explorer");
  ...
  const speakVoice = (text: string) => {
    ...
    try {
      ...
      const ukVoice = voices.find(
        (v) => v.lang.includes("uk") || v.lang.includes("UKR"),
      );
  ...
  const galaxyParticles = useMemo<GalaxyParticle[]>(() => {
    const labels = [
      { l: 'ТОВ "БМВ ГРУП"', t: "company" },
      { l: "Одеський Порт", t: "port" },
      { l: "Митна декл. №4039", t: "cargo" },
      { l: "Контейнер MSC-291", t: "container" },
      { l: "Коваленко О.М.", t: "person" },
      { l: "Німеччина (DE)", t: "country" },
      { l: 'АТ "Універсал Банк"', t: "bank" },
      { l: "Склад №4 Бровари", t: "warehouse" },
      { l: "Range Rover AA111", t: "auto" },
      { l: 'ТОВ "Арсенал"', t: "company" },
      { l: "Офшор Vanguard", t: "company" },
      { l: "Київська Митниця", t: "port" },
      { l: 'ДП "Антонов"', t: "company" },
      { l: "Митна декл. №2981", t: "cargo" },
      { l: "Контейнер COSCO", t: "container" },
      { l: "Китай (CN)", t: "country" },
      { l: 'АТ "ПриватБанк"', t: "bank" },
      { l: "Склад №12 Одеса", t: "warehouse" },
      { l: "Шевченко В.І.", t: "person" },
      { l: 'ТОВ "Технопостач"', t: "company" },
    ];
  ...
  useEffect(() => {
    ...
    if (selectedScenario) {
      ...
      const scenarioGreetings: { [key: string]: string } = {
        business:
          "Активовано бізнес-профіль суб'єктів. Перевіряю капітали, засновників та бенефіціарів.",
        logistics:
          "Завантажую сценарій Логістики. Оцінюю митні маршрути, транспорт та закордонні порти.",
        taxes:
          "Перевіряю податкові ризики. Співвідношу ПДВ та виявляю аномальні транзакції.",
        customs:
          "Запущено аналіз митних декларацій. Шукаю товари подвійного призначення.",
        geography: "Аналізую географічні зв'язки та транскордонні юрисдикції.",
        analytics:
          "Будую прогностичні ризики на основі ШІ-моделювання трендів.",
        assistant:
          "Привіт. Я твій персональний помічник Джарвіс. Готовий виконати будь-який аналіз.",
        partners:
          "Досліджую пов'язаних контрагентів, дочірні компанії та спільні активи.",
        risks:
          "Увага: увімкнено посилений санкційний скринінг. Перевіряю списки РНБО, OFAC та ЄС.",
      };

      const text =
        scenarioGreetings[selectedScenario] || "Зміна аналітичного сценарію.";
      speakVoice(text);
      if (activeEntity) {
  ...
  const buildGraphForEntity = (entity: OsintEntity) => {
    const centerNode: GraphNode = {
      ...
      sublabel: "NEXUS AI v3.5",
      type: "core",
      risk: "LOW",
      details:
        "Центральний обчислювальний модуль ШІ. Консолідує дані СБУ, ЄДР, митниці та судових реєстрів.",
    };
    ...
    if (
      ...
      secondaries.push({
        ...
        parentId: entity.id,
        details:
          entity.sanctions?.reason ||
          "Обмеження військово-промислового сектору",
      });
    }
  ...
  const handleSearchTrigger = (queryText: string) => {
    ...
    setTimeout(() => {
      ...
      if (matched) {
        ...
        speakVoice(
          `Знайдено компанію ${matched.name}. Сформовано граф зв'язків.`,
        );
      } else {
        const dummy: OsintEntity = {
          id: "custom-found",
          type: "company",
          name: queryText.toUpperCase().includes("ТОВ")
            ? queryText
            : `ТОВ "${queryText}"`,
          ...
          courts: {
            ...
            criminalCases: 1,
            lastCaseTitle:
              "Ухилення від сплати акцизних зборів при імпорті сировини",
          ...
          relationships: [
            {
              targetId: "person-custom",
              targetName: "Марченко Сергій Петрович",
              type: "OWNER_OF",
              risk: "MEDIUM",
            },
          ],
          aiRecommendations:
            "Рекомендується встановити червоний маркер ризику для митних декларацій. Провести аудит походження сировини.",
        };
        setActiveEntity(dummy);
        buildGraphForEntity(dummy);
        onSelectEntityGlobal(dummy);
        speakVoice(
          `Знайдено новий об'єкт ${dummy.name}. Виявлено прихований зв'язок з імпортом.`,
        );
      }
  ...
  const handleNodeClick = (node: GraphNode) => {
    ...
    if (node.type === "person") {
      ...
      additionalNodes.push({
        ...
        parentId: node.id,
        details:
          "Експортер мікроелектроніки подвійного призначення під санкціями США.",
      });
    }
    ...
    if (additionalNodes.length > 0) {
      ...
      additionalNodes.forEach((an, i) => {
        const offsetAngle =
          (i / additionalNodes.length) * Math.PI + Math.PI / 4;
  ...
  const handleWorkModeChange = (mode: typeof workMode) => {
    ...
    const modePhrases: { [key: string]: string } = {
      explorer:
        "Активовано режим дослідника. Виводжу повну деталізацію зв'язків.",
      business:
        "Активовано бізнес-режим. Мінімізую технічні деталі, показую лише відповіді.",
      expert:
        "Активовано режим експерта. Усі графи, реєстри та налаштування активні.",
      presentation:
        "Режим презентації активовано. Фокусую камеру на центральному інтелектуальному ядрі.",
      "command-center":
  ...
  return (
    <div
      className="relative min-h-[calc(100vh-100px)] flex flex-col justify-between"
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800 px-2 py-1.5 flex items-center justify-between text-xs font-mono">
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              ...
              if (!nextMuted) {
                ...
                setTimeout(
                  () => speakVoice("Голосовий асистент Джарвіс активовано."),
                  100,
                );
              }
            }}
          ...
          {bootProgress < 100 ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-300 text-xs truncate max-w-[210px]">
                Останнє сповіщення: "{lastSpokenText}"
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-xs text-slate-500 font-bold uppercase mr-1.5">
            РЕЖИМ РОБОТИ:
          </span>
          {[
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-2 items-stretch mt-12 w-full">
        ...
        >
          <div className="glass-panel-premium border-slate-800 p-2 rounded-2xl space-y-4 shadow-2xl shadow-black/40 backdrop-blur-sm">
            ...
            <div className="space-y-2 font-mono text-xs font-bold">
              {[
                {
                  id: "act-1",
                  text: "🔎 Проаналізувати компанію",
                  prompt: "ТОВ СпецТехПостач",
                },
                {
                  id: "act-2",
                  text: "🚛 Побудувати маршрут",
                  prompt: "Побудувати маршрут імпорту з Гонконгу",
                },
                {
                  id: "act-3",
                  text: "💰 Перевірити контрагента",
                  prompt: "Коваленко І.В.",
                },
                {
                  id: "act-4",
                  text: "📦 Знайти ризики",
                  prompt: "ТОВ Арсенал Сек'юріті",
                },
                {
                  id: "act-5",
                  text: "📈 Побудувати прогноз",
                  prompt: "Прогноз ризиків для оборонного замовлення",
                },
                {
                  id: "act-6",
                  text: "🌍 Показати мережу",
                  prompt: "Показати мережу офшорів бенефіціара",
                },
                {
                  id: "act-7",
                  text: "📄 Пояснити документ",
                  prompt: "Пояснити 16 томів комплаєнсу",
                },
              ].map((act) => (
                <button
                  key={act.id}
          ...
          <div className="bg-indigo-950/10 border border-indigo-900/20 p-2 rounded-2xl shadow-inner text-left space-y-2">
            ...
            <p className="text-xs text-indigo-200 leading-relaxed font-sans">
              "Для детального комплаєнсу рекомендується перевірити транскордонні
              митні декларації постачальників через <strong>Томи ТЗ 12</strong>.
              Особливу увагу зверніть на зв'язки в офшорних юрисдикціях."
            </p>
          </div>
        </div>
          ...
          <AnimatePresence>
            {suggestionState === "visible" && (
              ...
              >
                ...
                <div className="flex gap-1.5 shrink-0 text-xs font-mono font-black">
                  <button
                    onClick={() => {
                      setSuggestionState("dismissed");
                      setCoreState("validation");
                      speakVoice(
                        "Вивожу виявлені аномалії на граф. Зверніть увагу на червоні санкційні вузли.",
                      );
                      if (activeEntity) {
                        // High-risk focus
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const stateGreetings: { [key: string]: string } = {
                    ...
                    learning:
                      "Починаю процес навчання. Перебудовую нейронні ваги.",
                    optimization:
                      "Проводжу оптимізацію мережі зв'язків. Видаляю зайві вузли.",
                ...
                className={`px-2.5 py-1 rounded-2xl transition-all cursor-pointer ${coreState === s.id ? "bg-blue-600 text-white" : "text-slate-300 hover:text-slate-200"}`}
              >
                {s.label}
              </button>
            ))}
              ...
              {liveParticles.map((p) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`absolute rounded-full transition-opacity duration-300 ${isGalaxySwirling ? "bg-blue-400 opacity-90" : "bg-slate-700/40 opacity-40"}`}
                  >
                    {p.size > 5 && !isGalaxySwirling && (
                      <span className="absolute left-full ml-1 text-xs text-slate-500 font-mono scale-95 whitespace-nowrap block">
                        {p.label}
                      </span>
                    )}
                  </motion.div>
            ...
            {graphNodes.length > 0 && (
              ...
              >
                ...
                {graphNodes.map((node) => {
                  ...
                  const isLineActive =
                    coreState === "inference" || coreState === "validation";
                  const strokeCol =
                    node.risk === "HIGH"
                      ? "url(#glow-high)"
                      : node.risk === "MEDIUM"
                  ...
                  return (
                    <g key={`l-${node.id}`}>
                      <motion.line
                        x1={
                          parent.id === "center-ai"
                            ? parent.x! + mouseOffset.x
                            : parent.x
                        }
                        y1={
                          parent.id === "center-ai"
                            ? parent.y! + mouseOffset.y
                            : parent.y
                        }
                        x2={node.x}
                        y2={node.y}
                      ...
                      <circle
                        r={node.risk === "HIGH" ? "3.5" : "2"}
                        fill={node.risk === "HIGH" ? "#f43f5e" : "#6366f1"}
                      >
                        <animateMotion
            ...
            {graphNodes.map((node) => {
              ...
              if (isCenterAI) {
                ...
                const coreColors: { [key: string]: string } = {
                  idle: "from-indigo-900 via-indigo-700 to-blue-500 shadow-[0_0_40px_rgba(99,102,241,0.55)]",
                  learning:
                    "from-emerald-900 via-emerald-700 to-emerald-500 shadow-[0_0_45px_rgba(16,185,129,0.7)] border-emerald-400",
                  optimization:
                    "from-cyan-900 via-cyan-700 to-cyan-500 shadow-[0_0_35px_rgba(6,182,212,0.6)]",
                  inference:
                    "from-amber-900 via-amber-700 to-amber-500 shadow-[0_0_50px_rgba(245,158,11,0.7)] border-amber-400",
                  validation:
                    "from-purple-900 via-purple-700 to-purple-500 shadow-[0_0_45px_rgba(168,85,247,0.6)] border-purple-400",
                };
              ...
              return (
                ...
                >
                  ...
                  >
                    <div
                      className={`p-1 rounded shrink-0 ${node.risk === "HIGH" ? "bg-rose-500/15" : node.risk === "MEDIUM" ? "bg-amber-500/15" : "bg-blue-500/15"}`}
                    >
                      {node.type === "company" && (
                    ...
                    <div className="text-left overflow-hidden">
                      <h4 className="text-xs font-black tracking-tight text-white truncate max-w-[100px]">
                        {node.label}
                      </h4>
                      <p className="text-xs text-slate-500 truncate max-w-[100px] font-mono">
                        {node.sublabel}
                      </p>
                    </div>
                  </div>
            ...
            <div className="bg-slate-950/95 border border-slate-800 rounded-2xl flex items-center p-1.5 shadow-[0_15px_40px_rgba(0,0,0,0.6)]">
              ...
              {searchQuery && (
                <button
                  ...
                  className="px-2 text-xs text-slate-500 hover:text-slate-300 font-mono"
                >
                  CLEAR
                </button>
              )}
              <button
                ...
                className="bg-blue-600 hover:bg-blue-500 text-white font-mono font-bold text-xs uppercase px-2 py-1.5 rounded-2xl transition-all flex items-center gap-1"
              >
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-mono mt-0.5">
                          Код: {ent.code} • {ent.address.slice(0, 30)}
                        </p>
                      </div>
                      <span
                        className={`text-xs font-mono font-bold px-2 py-1 rounded ${ent.riskScore >= 80 ? "bg-rose-500/10 text-rose-400" : "bg-emerald-500/10 text-emerald-400"}`}
                      >
                        {ent.riskScore}% RISK
                      </span>
                    </button>
        ...
        <div className="xl:col-span-8 glass-panel-premium border-slate-800 rounded-2xl p-2 space-y-3 shadow-xl shadow-black/20">
          ...
          <div className="pt-2 flex items-center justify-between text-xs text-slate-500 font-mono border-t border-slate-800/60">
            <span>
              ...
              <strong
                className={
                  activeEntity?.status === "SANCTIONED"
                    ? "text-red-400 font-bold"
                    : "text-slate-300 font-bold"
                }
              >
                {activeEntity?.status || "Н/Д"}
              </strong>
            </span>
            <span>
              Клас ризику:{" "}
              <strong className="text-rose-400 font-bold">
                CLASS A HIGH COMPLIANCE
              </strong>
            </span>
        ...
        <div className="xl:col-span-4 bg-slate-900/40 border border-slate-800 rounded-2xl p-2 space-y-4 shadow-2xl shadow-black/40">
          <div className="flex justify-between items-start">
            ...
            >
              ...
              <span className="text-sm font-mono font-black tracking-tight leading-none block mt-1">
                {activeEntity?.riskScore || 0}%
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2 border-y border-slate-800 py-2 text-xs font-sans">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-slate-300 font-medium">
                {activeEntity?.founders?.[0]?.name || "Невідомо"}
              </span>
            </div>
          </div>
          ...
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-2">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-bold font-mono uppercase text-slate-200">
                  Хронологічний Таймлайн (Події об'єкта)
                </h4>
              </div>
              <button
                onClick={() => setShowTimelineDrawer(false)}
                className="text-xs text-slate-500 hover:text-slate-300 font-mono font-bold uppercase"
              >
                Закрити [X]
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-2 pt-1 font-sans">
              {[
                {
                  date: "2021-03-12",
                  title: "Заснування підприємства",
                  desc: "Реєстрація юрособи у м.Київ. Засновник — Коваленко І.В.",
                  risk: "LOW",
                },
                {
                  date: "2023-08-15",
                  title: "Офшорний Акціонер",
                  desc: "Передача 49% акцій Vanguard Holdings Ltd (Беліз).",
                  risk: "MEDIUM",
                },
                {
                  date: "2024-11-04",
                  title: "Збіг санкцій США",
                  desc: "Узгодження списків OFAC. Попередження системи.",
                  risk: "HIGH",
                },
                {
                  date: "2025-04-12",
                  title: "Кримінальне провадження",
                  desc: "Справа СБУ №42024. Обшуки, вилучення документації.",
                  risk: "HIGH",
                },
                {
                  date: "2026-05-10",
                  title: "Санкції РНБО України",
                  desc: "Офіційне внесення до санкційних списків.",
                  risk: "HIGH",
                },
              ].map((evt, idx) => (
                <div
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs font-mono px-2 py-1 rounded ${evt.risk === "HIGH" ? "bg-rose-500/10 text-rose-400" : evt.risk === "MEDIUM" ? "bg-amber-500/10 text-amber-400" : "bg-slate-800 text-slate-300"}`}
                    >
                      {evt.risk}
                    </span>
                  </div>
                  <h5 className="text-xs font-black text-slate-200 mt-1">
                    {evt.title}
                  </h5>
            ...
            <div>
              ...
              <div className="border border-dashed border-slate-800 bg-rose-500/5 p-2 rounded-2xl text-center space-y-1 mb-5">
                ...
                <p className="text-xs text-slate-300 font-mono">
                  ДОСТУП ДОЗВОЛЕНО ТІЛЬКИ ОРГАНАМ БЕЗПЕКИ УКРАЇНИ ТА ФІНАНСОВОМУ
                  МОНІТОРИНГУ
                </p>
              </div>
              ...
              <div className="space-y-4 text-xs text-slate-300">
                <div>
                  <h4 className="text-xs font-mono font-black text-blue-400 uppercase tracking-wider mb-1">
                ...
                <div>
                  <h4 className="text-xs font-mono font-black text-blue-400 uppercase tracking-wider mb-1">
                    2. 16 ТОМІВ ТЕХНІЧНОГО КОМПЛАЄНСУ
                  </h4>
                  <div className="bg-slate-900/50 rounded-2xl p-2 space-y-2">
                    <p className="leading-relaxed">
                      ШІ-алгоритм звірив контрактну документацію фірми із 16
                ...
                <div>
                  <h4 className="text-xs font-mono font-black text-blue-400 uppercase tracking-wider mb-1">
                    3. ШІ-ВИСНОВОК JARVIS
                  </h4>
                  <div className="bg-indigo-950/15 border border-indigo-900/40 p-2 rounded-2xl leading-relaxed italic text-indigo-200">
            ...
            <div>
              <div className="flex justify-between items-center border-b border-slate-800 pb-3 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black font-mono tracking-widest uppercase text-white">
                    ТАКТИЧНА КАРТА КАНАЛІВ ПОСТАЧАННЯ
                  </span>
                </div>
                <button
              ...
              <div className="w-full h-[240px] bg-black/40 backdrop-blur-md shadow-[0_4px_30px_rgba(30,58,138,0.1)] border border-slate-800 rounded-2xl relative overflow-hidden flex items-center justify-center mb-4">
                ...
                >
                  <path
                    d="M10 100 Q 80 50 150 120 T 290 80"
                    fill="none"
                    stroke="#f43f5e"
                    strokeWidth="2.5"
                    strokeDasharray="6 3"
