...
export function DataFlowInspector({ jobId }: DataFlowInspectorProps) {
  ...
  useEffect(() => {
    ...
    const pollBackend = async () => {
      ...
      try {
        let res: any = { message: 'Mock response' };
  ...
  const telemetry = { cpu: 0, ram: 0, io: 0, gpu: 0 };
  ...
  return (
    <div className="w-full flex flex-col gap-6 bg-[#05070f] border border-cyan-500/20 rounded-[2.5rem] p-6 shadow-[0_0_30px_rgba(6,182,212,0.05)] text-slate-300 font-sans relative overflow-hidden h-full">
      ...
      <div className="flex flex-row items-center justify-between pb-4 border-b border-white/[0.05] relative z-10">
        ...
        <div className="flex items-center gap-3">
          {stageIndex === -1 ? (
            <div className="bg-slate-500/10 border border-slate-500/30 text-slate-400 font-mono text-[9px] uppercase tracking-widest px-3 py-1">
              РЕЖИМ ОЧІКУВАННЯ
            </div>
          ) : (
            <div className="bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 font-mono text-[9px] uppercase tracking-widest px-3 py-1">
              {stageIndex === 6 ? 'КОНВЕЄР ГОТОВИЙ' : `ПРОГРЕС: ${progress}%`}
            </div>
          )}
          <button onClick={restartSimulation} className="bg-black/80 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-950/40 text-[10px] uppercase font-bold tracking-wider rounded-xl px-3 py-1 flex items-center">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center relative z-10">
                    ...
                    <div className={`text-[7px] font-black tracking-wider px-1.5 py-0.5 rounded ${info.active ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30' : 'bg-slate-800 text-slate-500 border border-white/5'}`}>
                      {info.label}
                    </div>
                  </div>
