/**
 * 📂 DOCUMENT REPOSITORY // СЕМАНТИЧНИЙ АРХІВ | v57.2-WRAITH
...
export const DocumentsTab = () => {
    ...
    return (
        <div className="space-y-12 pb-32">
                    ...
                    <div className="flex items-center gap-10">
                        ...
                        <div className="space-y-2">
                            ...
                            <h1 className="text-6xl font-black text-white tracking-tighter uppercase italic skew-x-[-2deg] leading-none mb-1">
                                АРХІВ <span className="text-yellow-500 underline decoration-yellow-600/20 decoration-8 italic uppercase">ЗНАНЬ</span>
                            </h1>
            ...
            {/* DROPZONE HUD */}
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative group border-2 border-dashed border-white/[0.04] hover:border-yellow-500/40 rounded-[4rem] p-16 transition-all duration-700 bg-black shadow-3xl cursor-pointer overflow-hidden">
                ...
                <div className="flex flex-col items-center justify-center text-center relative z-10 space-y-8">
                    <div className="w-24 h-24 rounded-[2rem] bg-yellow-600/10 border border-yellow-600/40 flex items-center justify-center shadow-3xl group-hover:scale-110 transition-transform duration-500">
                        <Upload size={48} className="text-yellow-400" />
                    </div>
                    <div className="space-y-4">
                        <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase group-hover:text-yellow-400 transition-colors leading-none">РОЗПОЧАТИ CЕМАНТИЧНУ ІНДЕКСАЦІЮ</h3>
            ...
            <div className="flex flex-col lg:flex-row gap-6 p-6 bg-black border-2 border-white/[0.03] rounded-[2.5rem] shadow-2xl relative z-20">
                <div className="flex flex-1 gap-6">
                    <div className="relative flex-1 group">
                        <Search className="absolute left-8 top-1/2 -translate-y-1/2 text-slate-800 group-focus-within:text-yellow-500 transition-colors" size={24} />
                        <input
                            type="text"
                            placeholder="ПОШУК_ЗА_НАЗВОЮ_АБО_МЕТАДАНИМИ..."
                            value={filter} onChange={e => setFilter(e.target.value)}
                            className="w-full bg-white/[0.02] border border-white/[0.04] rounded-2xl pl-20 pr-8 py-6 text-xl font-black text-white italic uppercase placeholder:text-slate-800 outline-none focus:border-yellow-500/40 transition-all font-mono"
                        />
                    </div>
                    <select
                    ...
                    <table className="w-full text-left">
                        <thead className="border-b border-white/[0.04] text-slate-700 text-[10px] font-black uppercase tracking-[0.4em] italic">
                            <tr>
                                ...
                                <th className="px-8 py-8 text-right underline decoration-yellow-500/20">CTRL</th>
                            </tr>
                        </thead>
                        <tbody className="text-sm">
                            <AnimatePresence mode="popLayout">
                                {loading && documents.length === 0 ? (
                                    <tr><td colSpan={6} className="py-40 text-center"><RefreshCw className="w-12 h-12 text-yellow-500 animate-spin mx-auto" /></td></tr>
                                    ...
                                    <motion.tr key={doc.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} className="border-b border-white/[0.02] hover:bg-white/[0.01] group/row transition-all">
                                        <td className="px-8 py-8">
                                            <div className="flex items-center gap-6">
                                                <div className="w-12 h-12 bg-yellow-600/10 border border-yellow-600/20 rounded-2xl flex items-center justify-center text-yellow-500 group-hover/row:bg-yellow-600 group-hover/row:text-black transition-all">
                                                    <FileText size={22} />
                                                </div>
                                                <div>
                                                    <p className="text-base font-black text-white italic truncate max-w-[300px] group-hover/row:text-yellow-400 transition-colors uppercase tracking-widest">{doc.title}</p>
                                                    <p className="text-[9px] font-black text-slate-700 uppercase italic mt-1 truncate max-w-[300px]">{doc.snippet}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-8">
                                            <Badge className="bg-yellow-600/10 text-yellow-500 border-none uppercase text-[9px] px-4 py-1 italic font-black">{doc.category.toUpperCase()}</Badge>
                                        </td>
