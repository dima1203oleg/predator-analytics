...
const ChaosControlHub: React.FC = () => {
  ...
  return (
    <div className="p-12 space-y-16 max-w-[1700px] mx-auto relative">
        ...
        <div className="flex flex-col gap-3 border-l-4 border-rose-600 pl-10 py-2">
          ...
          <div className="flex items-center gap-8 text-[11px] font-black font-mono text-white/30 tracking-[0.2em] uppercase italic">
            <div className="flex items-center gap-3">
              ...
              <span className="text-rose-500">П� ОТОКОЛ_ХАОСУ: АКТИВНИЙ_ОЧІКУВАННЯ</span>
            </div>
            <span className="opacity-20">•</span>
            <div className="flex items-center gap-3">
               <RefreshCw size={14} className="text-rose-500/60" />
               <span>ВЕ� ЕФІКАЦІЯ_SLA: Т� ИВАЄ</span>
            </div>
            <span className="opacity-20">•</span>
      ...
      {/* Cloud Infrastructure Sync (Colab & zrok) */}
      <div className="relative z-10 p-12 glass-wraith border-2 border-sky-500/30 rounded-[4rem] shadow-4xl overflow-hidden group">
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-8 p-12 glass-wraith border-2 border-white/5 rounded-[4rem] shadow-4xl relative overflow-hidden group hover:border-rose-500/20 transition-all duration-1000">
          ...
          <div className="flex items-center gap-8 mb-12 border-l-4 border-emerald-500 pl-10">
            ...
            <div>
              <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter leading-none glint-elite">МЕТ� ИКИ СТІЙКОСТІ_SLA</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
             ...
             <MetricRow label="� ІВЕНЬ_ПОМИЛОК_ERR" value={systemStatus?.overall_status === 'optimal' ? '0.00%' : 'Н/Д'} target="< 1.0%" status={systemStatus?.overall_status === 'optimal' ? "success" : "warning"} icon={Zap} />
             <MetricRow label="СТАН_ЗАПОБІЖНИКА" value={systemStatus?.healthy ? "ЗАК� ИТО" : "ВІДК� ИТО"} target="HEALTHY_L7" status={systemStatus?.healthy ? "success" : "danger"} icon={ShieldAlert} />
