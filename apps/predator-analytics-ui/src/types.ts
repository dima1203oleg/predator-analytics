...
export interface BrainNodeState { id: string; name: string; role: string; avatar: string; color: string; status: 'IDLE' | 'THINKING' | 'TALKING' | 'VOTING' | 'WAITING'; load?: number; }
...
export interface OSINTTool {
    id: string;
    name: string;
    category: 'РОЗВІДКА' | 'МЕ ЕЖА' | 'ДОКУМЕНТИ' | 'СОЦМЕ ЕЖІ' | string;
