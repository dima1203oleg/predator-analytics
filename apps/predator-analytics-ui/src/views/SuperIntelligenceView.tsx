...
const SuperIntelligenceView: React.FC = () => {
    ...
    return (
        <div className={`space-y-8 animate-in fade-in duration-500 w-full mx-auto relative z-10 ${isFocusMode ? 'fixed inset-0 z-[100] bg-slate-950 p-0 m-0 max-w-none backdrop-blur-3xl' : 'max-w-[1600px] pb-24'}`}>
            ...
            <div className={`grid gap-6 h-full transition-all ${isFocusMode ? 'grid-cols-1 p-8 h-screen' : 'grid-cols-1 lg:grid-cols-3 lg:h-[800px]'}`}>
                ...
                {!isFocusMode && (
                    <div className="flex flex-col gap-8 h-full order-1 lg:order-2">
                        ...
                        <TacticalCard variant="holographic" title="СИНАПСИС" className="flex-1 flex flex-col overflow-hidden glass-morphism panel-3d p-0" noPadding>
                            ...
                            <div className="flex-1 relative bg-slate-950/80 overflow-hidden">
                                <AnimatePresence mode="wait">
                                    {rightTab === 'STREAM' && (
                                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} key="stream" className="absolute inset-0 flex flex-col">
                                            <div className="h-[35%] border-b border-white/5 bg-slate-900/30 p-4 overflow-y-auto">
                                                <div className="text-[9px] text-indigo-400 font-black mb-3 flex items-center gap-3 sticky top-0 bg-[#0a0f1c] z-10 p-2 rounded-lg border border-white/5 uppercase tracking-[0.2em]"><Terminal size={12} /> КОМПІЛЯТОР ЯДРА NAS</div>
                                                <TypewriterBlock text={nasDiff || "--- ОЧІКУВАННЯ НЕЙРОННОГО СИНТЕЗУ ---"} isActive={stage === 'NAS_IMPLEMENTATION' || nasDiff === ''} />
                                            </div>
                                            <div className="flex-1 overflow-y-auto p-6 space-y-4" ref={logContainerRef}>
                                                {logs.length === 0 ? <div className="h-full flex flex-col items-center justify-center opacity-10"><Activity size={64} className="animate-pulse" /><span className="text-[10px] uppercase font-bold tracking-widest mt-4">Слухаю системну шину...</span></div> : logs.map(log => (
                                                    <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} key={log.id} className="flex gap-4 border-l-2 border-white/5 pl-4 py-1 hover:border-indigo-500 transition-all">
                                                        <span className="text-[8px] text-slate-700 font-mono mt-0.5">{log.timestamp}</span>
                                                        <span className={`text-[9px] font-black uppercase tracking-widest min-w-[50px] ${log.type === 'ERROR' ? 'text-rose-500' : log.type === 'SUCCESS' ? 'text-emerald-500' : 'text-indigo-400'}`}>{log.type}</span>
                                                        <span className="text-[10px] text-slate-300 font-medium leading-relaxed">{log.message}</span>
                                                    </motion.div>
                                                ))}
                                            </div>
                                        </motion.div>
                        ...
                        <TacticalCard variant="holographic" title="Арбітраж Рішень" className="h-[250px] glass-morphism panel-3d">
                            <ResponsiveContainer width="100%" height="100%">
