...
const INITIAL_BRAIN_NODES: BrainNodeState[] = [
    ...
    { id: 'arbiter', name: 'Gemini 3 Ultra', role: 'А БІТ ', avatar: 'A', color: '#ffffff', status: 'IDLE' }
];
...
const INITIAL_AGENTS: UnifiedAgentState[] = [
    ...
    { id: 'QA-TEST', name: 'QA  аннер', role: 'TESTER', status: 'IDLE' },
    { id: 'DEVOPS-01', name: 'GitOps Sync', role: 'EXECUTOR', status: 'IDLE' },
];

const INITIAL_GENOMES: AgentGenome[] = [
    { agentId: 'MON-01', version: 'v1.0.4', generation: 4, capabilities: [' озпізнавання патернів логів', 'Виявлення аномалій'], evolutionStatus: 'STABLE' },
...
const INITIAL_LOGS: SIContextLog[] = [
    { id: 'init-0', timestamp: '09:59:55', type: 'INFO', source: 'KERNEL', message: 'Завантаження Singularity Core v58.2-WRAITH...' },
...
// KERNEL: SINGULARITY v58.2-WRAITH
...
const SCENARIOS = [
    {
        ...
        verdict: "ЗАТВЕ ДЖЕНО: Переписати `search_tenders` використовуючи SQLAlchemy ORM.",
    ...
    },
    {
        ...
        triggerMsg: " озмір образу `predator-backend` перевищив 2.1 GB. Час завантаження pod > 45s.",
        ragDocs: [
        ...
        verdict: "А ХІТЕКТУ А: Впровадження Distroless Multi-stage Build.",
...
export const SuperIntelligenceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [logs, setLogs] = useState<SIContextLog[]>([
        { id: 'boot', timestamp: new Date().toLocaleTimeString(), type: 'INFO', source: 'KERNEL', message: 'Підключено до Predator v58.2-WRAITH | Нейронна Аналітика. Очікування потоку...' }
    ]);
                ...
                if (providers && providers.length > 0) {
                    const realBrainNodes: BrainNodeState[] = providers.map((p: any, idx: number) => {
                        ...
                        return {
                            id: p.id,
                            name: p.name,
                            role: existing?.role || (idx === 0 ? 'А БІТ ' : 'ЕКСПЕ Т'),
                            avatar: p.name[0],
