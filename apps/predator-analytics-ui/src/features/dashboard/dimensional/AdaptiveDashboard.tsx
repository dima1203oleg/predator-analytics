    ...
    try {
      ...
      switch (action) {
        ...
        case 'ПЕ ЕЗАПУСК':
          ...
          toast.info("ПЕ ЕЗАПУСК", "Процес перезапуску служб активовано");
          break;
        case 'Б АНДМАУЕ ':
...
function getDimensionTitle(dimension: string): string {
  switch (dimension) {
    case 'NEBULA': return 'П ОСТІ  NEBULA v45';
    case 'CORTEX': return 'МЕ ЕЖА CORTEX v45';
    case 'NEXUS': return 'КОМАНДНИЙ NEXUS v45';
    default: return 'П ИЛАДНА ПАНЕЛЬ v45';
  }
}
...
function getDimensionBreadcrumbs(dimension: string): string[] {
  switch (dimension) {
    ...
    case 'NEXUS': return ['КОМАНДНИЙ ЦЕНТ ', 'ПОВНИЙ КОНТ ОЛЬ'];
