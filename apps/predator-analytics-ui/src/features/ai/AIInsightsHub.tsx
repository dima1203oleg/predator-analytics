 ...
 * АНАЛІТИЧНИЙ_О� АКУЛ: Автономний синтез стратегічних інсайтів та прогнозів.
...
const TYPE_CONFIG: Record<string, { icon: any, color: string, label: string }> = {
  prediction:     { icon: Brain,       color: '#f43f5e', label: 'П� ОГНОЗ' },
  anomaly:        { icon: Activity,    color: '#e11d48', label: 'АНОМАЛІЯ' },
  opportunity:    { icon: Lightbulb,   color: '#fb7185', label: 'МОЖЛИВІСТЬ' },
  risk:           { icon: Shield,      color: '#9f1239', label: '� ИЗИК' },
  recommendation: { icon: Target,      color: '#f43f5e', label: '� ЕКОМЕНДАЦІЯ' },
...
const AIInsightsHub: React.FC<{ isWidgetMode?: boolean }> = ({ isWidgetMode = false }) => {
  ...
  const stats = useMemo(() => [
    { label: 'СТ� АТЕГІЧНИЙ ВПЛИВ', value: overview?.summary ? `$${(overview.summary.total_value_usd / 1e6).toFixed(1)}M` : '...', sub: 'П� ОЕКЦІЯ_ЕФЕКТУ_L7', icon: DollarSign, color: '#e11d48' },
    ...
    { label: 'АВТОНОМНІСТЬ ЯД� А', value: 'TIER-1', sub: 'S-� ІВЕНЬ_П� ОТОКОЛУ', icon: ShieldCheck, color: '#9f1239' },
  ], [overview]);
               ...
               <div className="flex-1 overflow-y-auto p-16 custom-scrollbar grid grid-cols-12 gap-16 relative z-10">
                  ...
                  <div className="col-span-4 space-y-10">
                     <div className="p-12 bg-rose-600/5 border-2 border-rose-500/20 rounded-[4rem] text-center space-y-6 shadow-4xl relative overflow-hidden group">
                        ...
                        <p className="text-[12px] font-black text-rose-500 uppercase tracking-[0.6em] italic">СТ� АТЕГІЧНИЙ_ВПЛИВ</p>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12 relative">
          <div className="flex items-center gap-12 border-l-4 border-rose-600 pl-12 py-4">
            ...
            <div className="space-y-4">
              ...
              <p className="text-[13px] text-white/40 font-black uppercase tracking-[0.6em] italic leading-none border-l-4 border-rose-900/50 pl-8">АВТОНОМНИЙ_СИНТЕЗ_ДЕ� ЖАВНОЇ_СТ� АТЕГІЇ_ТА_� ИЗИКІВ</p>
            </div>
          </div>
              ...
              <AnimatePresence mode="popLayout">
                {loading ? (
                  ...
                  <div className="py-40 text-center glass-wraith rounded-[4rem] border-2 border-dashed border-white/5 space-y-6">
                    ...
                    <span className="text-xl font-black text-white/20 uppercase tracking-[0.4em] italic block">НЕМАЄ_ІНСАЙТІВ_ЗА_ФІЛЬТ� ОМ</span>
                  </div>
                ) : filtered.map((insight: any, idx: number) => (
                  <motion.div
                    key={insight.id}
