...
const RECOMMENDATION_CFG: Record<MarketEntry['recommendation'], { label: string; color: string; bg: string; border: string; shadow: string; icon: any }> = {
  'strong-buy': { label: 'СУВЕРЕННИЙ ВХІД', color: '#e11d48', bg: 'bg-rose-950/20', border: 'border-cyan-500/40', shadow: '', icon: Star },
  'buy':        { label: 'РЕКОМЕНДОВАНО', color: '#fb7185', bg: 'bg-rose-900/20', border: 'border-cyan-500/30', shadow: 'shadow-none', icon: CheckCircle },
  'hold':       { label: 'СПОСТЕРЕЖЕННЯ',   color: '#94a3b8', bg: 'bg-slate-900/40',   border: 'border-slate-800/30', shadow: 'shadow-none', icon: Activity },
  'avoid':      { label: 'УНИКАТИ',      color: '#be123c', bg: 'bg-rose-950/40',     border: 'border-cyan-900/40',  shadow: 'shadow-none', icon: AlertTriangle },
};
    ...
    <PageTransition>
      <div className="min-h-screen bg-[#020202] text-slate-200 font-sans pb-32 relative overflow-hidden">
        ...
        <div className="pointer-events-none fixed inset-0 z-0">
          <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(6,182,212,0.05) 0%, transparent 55%)' }} />
        </div>
        ...
        <ViewHeader
          title={
            <div className="flex items-center gap-10">
              <div className="relative group">
                <div className="absolute inset-0 bg-cyan-500/20 blur-3xl rounded-full scale-150  group-hover:bg-cyan-500/30 transition-all duration-[5s]" />
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-6">
                  <span className={cn(
                    ...
                    isOffline ? "bg-cyan-500/10 border-cyan-500/20 text-cyan-500" : "bg-cyan-500/10 border-cyan-500/20 text-cyan-500"
                  )}>
                ...
                <h1 className="text-6xl font-black text-white tracking-tighter uppercase italic skew-x-[-3deg] leading-none">
                  MARKET ENTRY <span className={cn("underline decoration-[14px] underline-offset-[12px] italic uppercase tracking-tighter", isOffline ? "text-cyan-500 decoration-rose-500/20" : "text-cyan-500 decoration-rose-600/30")}>SCORE</span>
                </h1>
              </div>
            </div>
          }
          ...
          actions={
            <Button variant="cyber" className="px-14 py-6 bg-cyan-500 text-white text-[12px] font-black uppercase tracking-[0.4em] italic hover:brightness-110 transition-all rounded-[2rem] shadow-4xl flex items-center gap-4 font-bold">
              <Download size={24} />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            ...
            <motion.div
              ...
              className="p-10 bg-black/60  border-2 border-white/5 hover:border-cyan-500/30 transition-all rounded-[3.5rem] shadow-2xl group relative overflow-hidden"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="flex flex-wrap gap-4 items-center p-3 bg-black border-2 border-white/5 rounded-[2.5rem] w-fit shadow-4xl ">
              <div className="flex gap-2 bg-black border-2 border-white/5 p-2 rounded-2xl shadow-inner">
                {(['all', 'strong-buy', 'buy', 'hold'] as const).map(r => (
                  <Button variant="cyber" key={r} onClick={() => setFilterRec(r)}
              ...
              <div className="flex gap-2 bg-black border-2 border-white/5 p-2 rounded-2xl shadow-inner">
                <Button variant="cyber" onClick={() => setSortBy('score')} className={cn("px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] rounded-xl italic", sortBy === 'score' ? "text-cyan-500" : "text-slate-800")}>↓_SCORE</Button>
                <Button variant="cyber" onClick={() => setSortBy('growth')} className={cn("px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] rounded-xl italic", sortBy === 'growth' ? "text-cyan-500" : "text-slate-800")}>↓_GROWTH</Button>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {isLoading && (
                <div className="p-12 border-2 border-white/5 bg-black/40 rounded-[3.5rem] flex flex-col items-center gap-6">
                   ...
                   <span className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.5em]  italic">Синхронізація геополітичних векторів...</span>
                </div>
              )}

              {error && !isLoading && (
                <div className="p-12 border-2 border-cyan-500/20 bg-cyan-500/5 rounded-[3.5rem] flex flex-col items-center gap-6 text-center">
                   <AlertTriangle size={48} className="text-cyan-500 " />
                   ...
                   <Button variant="cyber" 
                     ...
                     className="px-8 py-3 bg-cyan-500/10 border border-cyan-500/40 text-cyan-500 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-cyan-500 hover:text-black transition-all"
                   >
                     RETRY_SYNC
                   </Button>
                </div>
                ...
                return (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      selected?.id === mkt.id
                        ? "bg-cyan-500/[0.03] border-cyan-500/30 shadow-4xl"
                        : "bg-black/60 border-white/5 hover:border-white/20"
                    )}
                  >
                    {selected?.id === mkt.id && (
                      <div className="absolute left-0 inset-y-0 w-2.5 bg-cyan-500 shadow-[0_0_20px_#e11d48]" />
                    )}
                    ...
                    <div className="flex items-start gap-8 pl-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-4 mb-2">
                          <h3 className="text-2xl font-black text-white group-hover:text-cyan-500 transition-colors uppercase italic tracking-tighter font-serif">{mkt.country}</h3>
                          <span className="text-[10px] text-rose-700 font-black uppercase tracking-widest bg-cyan-500/5 px-3 py-1 rounded-full">{mkt.region}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-10">
                          ...
                          <div>
                            ...
                            <p className="text-xl font-black text-cyan-600 font-mono italic">{mkt.growthRate}</p>
                          </div>
                        </div>
          ...
          <div className="lg:col-span-7">
            <AnimatePresence mode="wait">
              ...
              >
                {/* Заголовок ELITE */}
                <div className="bg-black/80  border-2 border-cyan-500/10 p-12 rounded-[4rem] shadow-4xl relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-32 opacity-[0.03] pointer-events-none">
                     <Globe size={400} className="text-cyan-500" />
                  </div>
                  <div className="flex items-start justify-between mb-12 relative z-10">
                    <div className="flex items-center gap-10">
                      ...
                      <div>
                        ...
                        <p className="text-[12px] text-cyan-600 font-black uppercase tracking-[0.5em] italic bg-cyan-500/5 px-6 py-2 rounded-full w-fit border border-cyan-500/10">{selected.region} · {selected.sector}</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-8 relative z-10">
                    {[
                      ...
                      <div key={i} className="p-8 border-2 border-white/5 bg-black rounded-[2.5rem] shadow-inner group hover:border-cyan-500/20 transition-all">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                  <div className="bg-black/60  border-2 border-white/5 p-10 rounded-[4rem] shadow-3xl relative overflow-hidden">
                    ...
                    <h3 className="text-[11px] font-black text-cyan-500/60 uppercase tracking-[0.6em] mb-10 italic">STRATEGIC_RADAR_SCAN</h3>
                    <div className="h-[320px] relative z-10">
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart data={radarData} cx="50%" cy="50%" outerRadius="80%">
                          <PolarGrid stroke="rgba(6,182,212,0.08)" strokeDasharray="5 5" />
                  ...
                  <div className="space-y-8">
                    {/* Можливості ELITE */}
                    <div className="bg-black/60 border-2 border-cyan-500/10 p-10 rounded-[3.5rem] group hover:border-cyan-500/30 transition-all shadow-3xl relative overflow-hidden">
                       <div className="absolute top-0 left-0 w-1.5 h-full bg-cyan-500 opacity-20" />
                      <h3 className="text-[11px] font-black text-cyan-500/60 uppercase tracking-[0.6em] mb-8 flex items-center gap-4 italic">
                        <CheckCircle size={20} className="text-cyan-500 " />
                        ALPHA_GROWTH_VESTORS
                      </h3>
                      <div className="space-y-4">
                        {selected.opportunities.map((o: string, i: number) => (
                          <div key={i} className="flex gap-4 p-5 bg-cyan-500/[0.02] border border-white/5 rounded-2xl group-hover:bg-cyan-500/[0.04] transition-all">
                            <ArrowUpRight size={18} className="text-cyan-600 mt-1 shrink-0" />
                ...
                <div className="bg-black/60 border-2 border-white/5 p-12 rounded-[4rem] shadow-4xl grid grid-cols-1 xl:grid-cols-2 gap-12 ">
                  <div>
                    <h3 className="text-[11px] font-black text-slate-700 uppercase tracking-[0.6em] mb-8 flex items-center gap-4 italic">
                      <Building2 size={20} className="text-cyan-600/40" />
                      STRATEGIC_LOCAL_NODES
                    </h3>
                    <div className="space-y-4">
                      {selected.localPartners.map((p: string, i: number) => (
                        <div key={i} className="flex items-center gap-6 p-6 border-2 border-white/5 bg-black rounded-3xl group hover:border-cyan-500/30 transition-all cursor-crosshair shadow-inner">
                          <Building2 size={24} className="text-slate-800 group-hover:text-cyan-500 transition-colors" />
                          ...
                          <ArrowUpRight size={20} className="ml-auto text-slate-900 group-hover:text-cyan-600 transition-colors" />
                        </div>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-4">
                      {selected.entryMode.map((e: string, i: number) => (
                        <div key={i} className="flex items-center gap-6 p-6 border-2 border-white/5 bg-black rounded-3xl shadow-inner group hover:bg-cyan-500/[0.01] transition-all cursor-default">
                          <div className="p-2 bg-cyan-500/5 rounded-lg border border-cyan-500/10"><CheckCircle size={20} className="text-rose-700 group-hover:text-cyan-500 transition-all" /></div>
                ...
                <div className="relative group overflow-hidden rounded-[5rem] border-2 border-cyan-500/30 bg-gradient-to-br from-rose-500/10 via-black/40 to-[#020202] p-12 shadow-4xl ">
                    <div className="absolute top-0 right-0 p-16 opacity-10 pointer-events-none group-hover:scale-125 transition-transform duration-[10s]">
                        <Zap size={300} className="text-cyan-500" />
                    </div>
                    <div className="relative z-10 space-y-10">
                        <div className="flex items-center gap-8">
                            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-[2rem] bg-cyan-600 text-white ">
                                <Target size={40} />
                            </div>
                            <div>
                        ...
                        <p className="text-lg leading-[2.2] text-slate-300 italic border-l-8 border-cyan-600/50 pl-12 bg-white/[0.02] py-10 rounded-r-[4rem] font-medium shadow-inner">
                ...
                <div className="pt-8">
                  <Button variant="cyber" className="w-full py-10 bg-gradient-to-r from-rose-700 via-rose-600 to-rose-700 text-white text-[14px] font-black uppercase tracking-[0.6em] italic hover:brightness-110 transition-all shadow-4xl rounded-[3rem] group flex items-center justify-center gap-8 border-4 border-cyan-500/20">
      ...
      <style dangerouslySetInnerHTML={{ __html: `.custom-scrollbar::-webkit-scrollbar{width:6px}.custom-scrollbar::-webkit-scrollbar-track{background:transparent}.custom-scrollbar::-webkit-scrollbar-thumb{background:rgba(6,182,212,.15);border-radius:20px;border:2px solid black}.custom-scrollbar::-webkit-scrollbar-thumb:hover{background:rgba(6,182,212,.3)}` }} />
    </PageTransition>
  );
};
