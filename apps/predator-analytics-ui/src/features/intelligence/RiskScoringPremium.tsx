...
const RISK_CONFIG: Record<string, { label: string; color: string; bg: string; border: string; glow: string; icon: any }> = {
  critical: { label: 'КРИТИЧНИЙ', color: '#E11D48', bg: 'bg-rose-950/20', border: 'border-cyan-500/40', glow: '', icon: XCircle },
  high: { label: 'ВИСОКИЙ', color: '#BE123C', bg: 'bg-rose-900/20', border: 'border-cyan-500/40', glow: '', icon: AlertTriangle },
  medium: { label: 'СЕРЕДНІЙ', color: '#FB7185', bg: 'bg-rose-900/20', border: 'border-cyan-500/40', glow: '', icon: AlertCircle },
  ...
  elevated: { label: 'ПІДВИЩЕНИЙ', color: '#F43F5E', bg: 'bg-rose-900/20', border: 'border-cyan-500/40', glow: 'shadow-none', icon: AlertTriangle },
};
...
const ScanningHUD: React.FC = () => {
    return (
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 opacity-20">
            ...
            />
            <div className="absolute top-40 left-10 flex flex-col gap-2 font-mono text-[7px] text-cyan-500/30 uppercase italic">
                <span>ARRAY_STATUS: ACTIVE</span>
                <span>FAILOVER_READY: STANDBY</span>
                <span>COGNITIVE_ENGINE: ELITE_58.2</span>
            </div>
            <div className="absolute bottom-40 right-10 flex flex-col gap-2 font-mono text-[7px] text-cyan-500/30 uppercase italic text-right">
...
const RiskCognitiveParser: React.FC = () => {
    ...
    return (
        <div className="w-full h-48 bg-black/60 border-2 border-cyan-500/10 rounded-[2.5rem] p-6 font-mono text-[9px] overflow-hidden relative group">
            <div className="absolute top-4 right-6 flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-cyan-500 animate-ping" />
                <span className="text-cyan-500 font-black italic tracking-widest text-[8px]">COGNITIVE_RISK_PARSER</span>
            </div>
            <div className="space-y-1 opacity-50">
                {lines.map((line, i) => (
                    <motion.div 
                        ...
                        className={cn("uppercase", line.includes('CRITICAL') ? "text-cyan-500 font-bold" : "text-slate-500")}
                    >
    ...
    try {
      ...
      if (!isOffline && mappedEntities.length > 0) {
        ...
        window.dispatchEvent(new CustomEvent('predator-error', {
          detail: {
            service: 'RiskProtocol',
            message: `СИНХ ОНІЗАЦІЮ RISK_ENGINE_SYNC ЗАВЕ ШЕНО [${nodeSource}]: Оброблено ${mappedEntities.length} суб'єктів під кворумом. Протокол моніторингу стабілізовано.`,
            severity: 'info',
            timestamp: new Date().toISOString(),
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-[#020202] text-slate-200 relative overflow-hidden font-sans pb-40 px-4 xl:px-12 pt-12">
        ...
        <div className="absolute inset-x-0 top-0 h-[800px] bg-[radial-gradient(circle_at_50%_0%,rgba(6,182,212,0.08),transparent_70%)] pointer-events-none" />
        ...
        <div className="relative z-10 max-w-[1850px] mx-auto space-y-16 flex flex-col items-stretch">
            ...
            title={
              <div className="flex items-center gap-12">
                <div className="relative group">
                  <div className="absolute inset-0 bg-cyan-600/20 blur-[80px] rounded-full scale-150 " />
                  <div className="relative p-8 bg-black border-2 border-cyan-500/40 rounded-[3rem] shadow-4xl transform -rotate-3 hover:rotate-0 transition-all duration-700">
                    <ShieldAlert size={48} className="text-cyan-500 shadow-[0_0_30px_#e11d48]" />
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <h1 className="text-7xl font-black text-white tracking-tighter uppercase italic skew-x-[-4deg] leading-none">
                    РИЗИК <span className="text-cyan-500 underline decoration-rose-500/30 decoration-[16px] underline-offset-[16px] italic uppercase tracking-tighter">СКО ИНГ</span>
                  </h1>
                </div>
              </div>
            }
          ...
          <div className="grid grid-cols-12 gap-10">
              ...
              <div className="col-span-12 xl:col-span-4 flex items-center justify-end">
                  <div className="text-right space-y-2">
                      <p className="text-[10px] font-black text-cyan-500/40 uppercase tracking-[0.4em] italic">MONITOR_STATUS</p>
          ...
          <div className="flex justify-end gap-6">
             <Button variant="cyber" 
              ...
              className={cn(
                "p-7 bg-black border-2 border-white/[0.04] rounded-[2rem] text-slate-500 hover:text-cyan-500 transition-all shadow-4xl group/btn",
              ...
              {[
                ...
                <Button variant="cyber" 
                   ...
                   className={cn(
                     ...
                     selectedLevel === m.level ? "border-cyan-500 -slow" : "border-white/[0.03] hover:border-white/10"
                   )}
                >
          ...
          <div className="grid grid-cols-12 gap-12">
            ...
            <div className="col-span-12 xl:col-span-8 space-y-12">
               <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-20 pr-10 py-8 bg-black border-2 border-white/[0.04] rounded-[2.5rem] text-xl font-black italic text-white placeholder:text-slate-900 focus:outline-none focus:border-cyan-500/30 transition-all shadow-inner tracking-tighter"
                  />
                  <div className="absolute right-8 top-1/2 -translate-y-1/2 flex items-center gap-4">
                     ...
                     <div className="p-3 bg-cyan-500/10 border border-cyan-500/20 rounded-xl text-cyan-500"><Scan size={18} /></div>
                  </div>
               </div>
               ...
               <div className="p-16 rounded-[5rem] bg-black border-2 border-white/[0.04] shadow-4xl space-y-12 relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-16 opacity-[0.02]">
                     <Orbit size={320} className="text-cyan-500 animate-spin-slow" />
                  </div>
                  <div className="flex items-center justify-between border-b-2 border-white/[0.04] pb-10 relative z-10">
                     <h3 className="text-2xl font-black text-white italic uppercase tracking-[0.5em] flex items-center gap-6">
                        <Database size={32} className="text-cyan-500" /> РЕЄСТР_СУБ'ЄКТІВ_ПІД_КВОРУМОМ
                     </h3>
                     <div className="flex gap-4">
                        <Button variant="cyber" className="p-4 bg-white/[0.02] border-2 border-white/5 rounded-2xl text-slate-700 hover:text-white transition-all"><Filter size={20} /></Button>
                        <Button variant="cyber" className="p-4 bg-white/[0.02] border-2 border-white/5 rounded-2xl text-slate-700 hover:text-white transition-all"><Layers size={20} /></Button>
                     </div>
                  </div>
                     ...
                     {loading ? (
                       ...
                       filteredEntities.map((entity) => (
                         <div 
                           ...
                           className={cn(
                               "p-10 rounded-[4rem] bg-white/[0.01] border-2 border-white/[0.03] hover:bg-cyan-500/[0.02] hover:border-cyan-500/30 transition-all duration-700 group cursor-pointer relative overflow-hidden",
                         ...
                         >
                            {entity.riskLevel === 'critical' && (
                                <motion.div 
                                    ...
                                    className="absolute inset-x-0 bottom-0 h-1 bg-cyan-600/50"
                                />
                            )}
                            <div className="flex items-center justify-between">
                               <div className="flex items-center gap-10">
                                  ...
                                  <div className="space-y-3">
                                     <div className="flex items-center gap-4">
                                        <h4 className="text-3xl font-black text-white italic uppercase tracking-tighter group-hover:text-cyan-500 transition-all leading-none">{entity.name}</h4>
                                     ...
                                     <div className="flex gap-10 text-[11px] font-black text-slate-800 uppercase italic tracking-widest leading-none">
                                        ...
                                        <span className="flex items-center gap-2 pb-1 border-b border-cyan-500/20"><Activity size={12} className="text-cyan-500" /> ОПЕРАЦІЙ: {entity.totalOperations}</span>
                                     </div>
                                  </div>
                               </div>
                               <div className="flex gap-8 items-center">
                                  ...
                                  <div className="p-4 bg-black border-2 border-white/5 rounded-3xl group-hover:bg-cyan-600 group-hover:border-cyan-500 transition-all">
                            ...
                            <div className="mt-8 flex gap-3">
                               {entity.flags.map(f => (
                                 <span key={f} className="text-[9px] font-black text-slate-800 bg-black/40 border-2 border-white/5 px-4 py-1.5 rounded-xl uppercase italic tracking-widest group-hover:text-rose-400 group-hover:border-cyan-500/20 transition-all">#{f}</span>
                               ))}
                            </div>
                         </div>
                       ))
            ...
            <div className="col-span-12 xl:col-span-4 space-y-12">
               ...
               <div className="p-12 rounded-[5rem] bg-gradient-to-br from-rose-700/10 to-rose-900/10 border-4 border-cyan-600/20 shadow-4xl space-y-10 relative overflow-hidden group/ai">
                  <div className="absolute top-0 right-0 p-12 opacity-[0.03] group-hover/ai:opacity-[0.1] transition-opacity duration-1000">
                     <Microscope size={180} className="text-cyan-500 " />
                  </div>
                  <div className="flex items-center gap-6 border-b-2 border-white/[0.05] pb-8 relative z-10">
                    <div className="p-5 bg-cyan-600/10 border-2 border-cyan-600/20 rounded-[2rem] text-cyan-500">
                       <Scale size={32} />
                    </div>
                    <div className="space-y-1">
                       <h3 className="text-xl font-black text-white italic uppercase tracking-[0.5em]">ЦЕНТ _РОЗСЛІДУВАНЬ</h3>
                  ...
                  <div className="space-y-6 relative z-10 pt-4 font-black italic">
                     {[
                       { t: 'П ОЦЕСУАЛЬНА_ДІЯ', v: 'Відкрито справу про дроблення бізнесу // [CASE_4282]', cl: 'text-cyan-500', i: Flag },
                       { t: 'СИГНАЛ_МОНІТОРИНГУ', v: 'Аномальні виплати на користь офшорів (Кіпр) // TRIGGER_7', cl: 'text-cyan-600', i: AlertTriangle },
                       { t: 'ВЕ ИФІКАЦІЯ_UBO', v: 'Зміна бенефіціара П АТ "ЕНЕ ГО" на номінальну особу', cl: 'text-rose-700', i: Users },
                     ].map((ins, i) => (
                       <div key={i} className="p-8 rounded-[2.5rem] bg-black/60 border-2 border-white/[0.03] space-y-4 group/item hover:border-cyan-500/30 transition-all duration-500 shadow-inner">
                          <div className="flex items-center gap-4">
                  ...
                  <Button variant="cyber" className="w-full py-8 bg-cyan-600 text-white rounded-[2.5rem] text-[12px] font-black uppercase tracking-[0.5em] italic shadow-4xl hover:bg-cyan-500 hover:scale-[1.02] active:scale-95 transition-all duration-500 relative z-10 border-4 border-cyan-400/20">
               ...
               <div className="p-12 rounded-[5rem] bg-black border-2 border-white/[0.04] shadow-4xl space-y-12 relative overflow-hidden">
                  ...
                  <div className="space-y-6 relative z-10 pt-4 font-black">
                     {[
                       { i: Network, l: 'МАТрИЦЯ_ЗВ\'ЯЗКІВ', c: 'text-cyan-500', sub: 'GRAPH_ENGINE_ANALYSIS' },
                       { i: Target, l: 'АВТО_СКАНЕ _СХЕМ', c: 'text-cyan-600', sub: 'PATTERN_RECOGNITION' },
                       ...
                       <Button variant="cyber" key={i} className="w-full flex items-center justify-between p-8 rounded-[3rem] bg-white/[0.01] border-2 border-white/[0.03] hover:bg-white/[0.04] hover:border-white/10 transition-all duration-500 group/act shadow-xl italic uppercase">
                          ...
                          <ChevronRight size={20} className="text-slate-900 group-hover/act:text-cyan-500 transition-all group-hover/act:translate-x-2" />
                       </Button>
                     ))}
                  </div>
               </div>
          ...
          {selectedEntity && (
            <>
              ...
              <motion.div
                ...
                className="fixed top-0 right-0 bottom-0 w-full max-w-5xl bg-[#050505] border-l-4 border-cyan-600/30 z-[101]  flex flex-col overflow-hidden"
              >
                <div className="p-12 border-b-2 border-white/[0.04] flex items-center justify-between bg-black relative">
                   <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(6,182,212,0.05),transparent_70%)]" />
                   <div className="flex items-center gap-10 relative z-10">
                      <div className="relative">
                         <div className="absolute inset-0 bg-cyan-600/30 blur-3xl " />
                         <div className="w-32 h-32 flex items-center justify-center p-4 bg-black border-2 border-cyan-500/40 rounded-[2.5rem] shadow-4xl text-5xl font-black italic font-mono" style={{ color: RISK_CONFIG[selectedEntity.riskLevel]?.color }}>{selectedEntity.riskScore}</div>
                      </div>
                      <div>
                         ...
                         <div className="flex items-center gap-6">
                            <span className="text-cyan-500 font-mono text-[14px] font-black uppercase tracking-widest italic">ЄДРПОУ: {selectedEntity.edrpou}</span>
                   ...
                   <Button variant="cyber" onClick={() => setSelectedEntity(null)} className="p-6 bg-white/[0.02] hover:bg-cyan-600/20 text-slate-800 hover:text-white rounded-[2rem] transition-all border-2 border-white/5 hover:border-cyan-500 relative z-10"><XCircle size={32} /></Button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar p-16 space-y-20 relative">
                   ...
                   <section className="space-y-12 relative z-10">
                      <div className="flex items-center gap-6">
                         ...
                         <h3 className="text-[12px] font-black text-cyan-500 uppercase tracking-[0.6em] italic font-serif">CERS 5-ША ОВА МОДЕЛЬ РИЗИКУ</h3>
                   ...
                   <section className="space-y-12 relative z-10">
                      <div className="flex items-center gap-6">
                         <div className="h-px flex-1 bg-gradient-to-r from-transparent to-rose-500/30" />
                         <h3 className="text-[12px] font-black text-cyan-500 uppercase tracking-[0.6em] italic font-serif">ВЕ ДИКТ SOVEREIGN ADVISOR</h3>
                         <div className="h-px flex-1 bg-gradient-to-l from-transparent to-rose-500/30" />
                      </div>
                ...
                <div className="p-12 bg-black border-t-2 border-white/[0.04] flex gap-8 relative z-10">
                   <Button variant="cyber" className="flex-1 py-10 bg-cyan-600 text-white rounded-[2.5rem] font-black italic uppercase text-[13px] tracking-[0.4em] transition-all shadow-4xl hover:scale-[1.02] border-4 border-cyan-400/20 active:scale-95">
                      СФО МУВАТИ_VIP_АНАЛІЗ
                   </Button>
                   <Button variant="cyber" className="flex-1 py-10 bg-black border-2 border-white/10 text-slate-600 hover:text-white hover:border-cyan-500/40 rounded-[2.5rem] font-black italic uppercase text-[12px] tracking-[0.3em] transition-all">
                      СИГНАЛ_СЕК'Ю ІТІ
                   </Button>
                </div>
              </motion.div>
