    ...
    return (
      ...
      >
        <button 
          ...
          className="flex items-center gap-2 text-white/50 hover:text-rose-500 transition-colors uppercase font-black tracking-widest text-xs"
        >
          <ArrowLeft size={16} /> НАЗАД ДО СПИСКУ
        </button>

        <div className="p-6 rounded-3xl border-2 border-rose-500/20 bg-rose-500/5 shadow-lg flex flex-col gap-4">
          <div className="flex items-start justify-between">
            <h2 className="text-2xl font-black italic tracking-tighter uppercase leading-none">{selectedSubject.name}</h2>
            <span className={cn("px-3 py-1 rounded text-[9px] font-black uppercase tracking-widest", selectedSubject.risk === 'Підтверджено' ? "bg-rose-500/20 text-rose-500" : "bg-white/10 text-slate-300")}>
          ...
          <div className="grid grid-cols-2 gap-4 mt-2">
            ...
            <div className="flex flex-col gap-1 p-3 bg-black/40 rounded-xl">
              ...
              <span className="text-xs font-black text-rose-500">{selectedSubject.competitor}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4">
            <Crosshair size={20} className="text-rose-600 animate-pulse" />
          ...
          {selectedSubject.signals.map((signal) => (
            <div key={signal.id} className="p-5 bg-black/60 border border-white/10 rounded-2xl flex flex-col gap-3 shadow-md">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-white/5 rounded-xl text-rose-500">
                ...
                <div className="flex flex-col flex-1">
                  <div className="flex justify-between w-full">
                    ...
                    <span className="text-[9px] font-black text-rose-500">{signal.confidence}% CONF</span>
                  </div>
              ...
              <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                 <div className={cn("h-full", signal.confidence > 80 ? "bg-rose-500" : "bg-rose-500/40")} style={{ width: `${signal.confidence}%` }} />
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="flex flex-col gap-6 p-4 pb-24 overflow-y-auto h-full">
      <div className="flex flex-col gap-2">
        <h2 className="text-3xl font-black text-white uppercase tracking-tighter italic text-shadow-glow-rose leading-none">
          СИСТЕМА <span className="text-rose-500">ЗРАДА</span>
        </h2>
        <div className="flex items-center gap-2 px-3 py-1 bg-rose-500/10 border border-rose-500/30 rounded-lg self-start mt-2">
          <div className="w-2 h-2 bg-rose-500 rounded-full animate-ping" />
          <span className="text-[10px] font-black text-rose-500 tracking-widest uppercase italic">МОНІТОРИНГ ДОБРОЧЕСНОСТІ</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex flex-col p-4 bg-black/60 border border-rose-500/20 rounded-2xl">
          <div className="flex items-center justify-between mb-2 opacity-50">
            <span className="text-[9px] font-bold uppercase tracking-widest text-rose-500">ЗРАДА</span>
            <Skull size={14} className="text-rose-500" />
          </div>
          <span className="text-2xl font-black text-rose-500">{subjects.filter(s => s.risk === 'Підтверджено').length}</span>
        </div>
      </div>

      <div className="flex gap-4">
        <button onClick={() => { play(UISoundType.CLICK); runDeepOsint(); }} className="flex-1 py-4 bg-rose-900/20 border border-rose-500/30 text-rose-500 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2">
           <Scan size={14} /> {isOsintLoading ? 'СКАНУВАННЯ...' : 'СКРІНІНГ'}
        </button>
        <button onClick={() => play(UISoundType.CLICK)} className="flex-1 py-4 bg-rose-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-rose-900/50">
           <Plus size={14} /> ДОДАТИ
        </button>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 mt-2">
        {['Всі', 'Підтверджено', 'Висока підозра', 'Моніторинг'].map(r => (
          <button key={r} onClick={() => { play(UISoundType.CLICK); setFilterRisk(r as any); }} className={cn("px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all", filterRisk === r ? "bg-rose-600 border-rose-400 text-white" : "bg-black text-slate-500 border-white/10")}>
            {r}
          </button>
        ))}
      </div>
        ...
        {filtered.map((subject) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={cn("p-3 rounded-xl border", subject.risk === 'Підтверджено' ? "bg-rose-500/10 border-rose-500/30 text-rose-500" : "bg-white/5 border-white/10 text-white/50")}>
                  <UserX size={20} />
                </div>
            ...
            <div className="flex items-center justify-between border-t border-white/5 pt-4 mt-2">
              <span className={cn("px-3 py-1 rounded text-[9px] font-black uppercase tracking-widest", subject.risk === 'Підтверджено' ? "bg-rose-500/20 text-rose-500" : "bg-white/10 text-slate-300")}>
                {subject.risk}
              </span>
              <div className="flex items-center gap-2">
                <span className="text-xl font-black text-rose-500 italic leading-none">{subject.evidenceCount}</span>
