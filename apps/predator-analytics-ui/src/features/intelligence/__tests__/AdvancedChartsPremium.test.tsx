...
describe('AdvancedChartsPremium', () => {
    ...
    test('відмальовує заголовок та перемикачі діапазону', async () => {
        ...
        expect(screen.getByText(/РІК_2026/i)).toBeInTheDocument()
    })
    ...
    test('завантажує та відображає дані на графіках', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText(/ДИНАМІКА РИНКОВИХ ТРЕНДІВ/i)).toBeInTheDocument()
            expect(screen.getByText(/РОЗПОДІЛ КАТЕГОРІЙ/i)).toBeInTheDocument()
