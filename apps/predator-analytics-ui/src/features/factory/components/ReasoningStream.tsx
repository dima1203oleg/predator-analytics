      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/5">
        ...
        {isStreaming && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-[8px] font-black text-cyan-500 uppercase tracking-widest">Інференс_Активний</span>
          </div>
        )}
      </div>
            ...
            steps.map((step, idx) => (
              <motion.div
                ...
                className={cn(
                  ...
                  idx === steps.length - 1 ? "border-cyan-500/50" : "border-white/10"
                )}
              >
                {/* Dot */}
                <div className={cn(
                  "absolute left-[-5px] top-1 w-2 h-2 rounded-full",
                  idx === steps.length - 1 ? "bg-cyan-500 " : "bg-white/20"
                )} />

                <div className="space-y-2">
                  <div className="flex items-center gap-3 text-[9px] font-mono text-slate-500">
                    <span className="text-cyan-500/60">[{new Date(step.timestamp).toLocaleTimeString()}]</span>
                  ...
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/5 font-mono text-[11px] leading-relaxed text-slate-300">
                    <span className="text-cyan-500/40">{'<thinking>'}</span>
