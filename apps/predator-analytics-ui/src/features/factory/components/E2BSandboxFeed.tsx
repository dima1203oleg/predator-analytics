    ...
    <div className={cn("flex flex-col h-full bg-black/60 rounded-[28px] border border-white/5 overflow-hidden", className)}>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-emerald-500/5">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className={cn(
            ...
            session.status === 'ACTIVE' ? "text-emerald-500" : "text-cyan-500"
          )}>
            <div className={cn("w-1.5 h-1.5 rounded-full", session.status === 'ACTIVE' ? "bg-emerald-500 " : "bg-cyan-500")} />
        ...
        {session.logs.map((log, i) => {
          ...
          return (
            <div key={i} className={cn(
              ...
              isWarning ? "bg-cyan-500/10 text-rose-400 border-l-2 border-cyan-500" :
      ...
      <div className="px-6 py-3 border-t border-white/5 flex items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[8px] font-black text-cyan-500 uppercase tracking-[0.2em]">Монітор_Цілісності_Пісочниці: НОРМА</span>
        </div>
      </div>
    </div>
  );
