    ...
    >
        ...
        <div className="flex-1 space-y-2">
            ...
            <div className="flex items-center gap-4 text-[9px] text-slate-700 font-black uppercase italic">
                ...
                <span>� ИЗИК: <span className={cn(node.riskScore > 70 ? 'text-amber-500' : 'text-emerald-500')}>{node.riskScore}%</span></span>
            </div>
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            setData({
                ...
                insights: [
                    { question: 'ХТО КОНТ� ОЛЮЄ ТИТАНОВИЙ ШЛЕЙФ?', answer: 'Група "Оріон" через 3 офшори.', type: 'critical' },
                       ...
                       <div className="flex items-center gap-10">
                          ...
                          <div className="space-y-2">
                             ...
                             <p className="text-[11px] text-slate-500 font-black uppercase tracking-[0.4em] italic opacity-80 leading-none">
                               ВЕ� ТИКАЛЬ КОНТ� ОЛЮ, ТІНЬОВІ БЕНЕФІЦІА� И ТА ЛОБІЗМ
                             </p>
                          </div>
                       </div>
                     }
                         ...
                         <div className="p-12 rounded-[4rem] bg-black border-2 border-white/[0.04] shadow-3xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10 space-y-12">
                               <div className="flex items-center gap-6 pb-8 border-b border-white/[0.04]">
                                  ...
                                  <h3 className="text-3xl font-black text-white italic uppercase tracking-widest skew-x-[-2deg]">ВЕ� ТИКАЛЬ_ЦЕНТ� АЛЬНОГО_КОНТ� ОЛЮ</h3>
                               </div>
                                       ...
                                       <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                                          {data.levels.level3.nodes.map(node => (
                                            <div key={node.id} className="p-6 rounded-3xl bg-white/[0.01] border border-white/[0.04] text-center hover:border-cyan-500/30 transition-all group/it">
                                               ...
                                               <p className="text-[9px] font-black text-slate-700 uppercase italic">Вплив {node.power}% | � изик {node.riskScore}%</p>
                                            </div>
                                          ))}
                                       </div>
                                    </div>
