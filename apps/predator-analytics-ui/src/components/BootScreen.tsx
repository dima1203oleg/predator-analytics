/**
 * BootScreen — PREDATOR NEXUS v61.0-ELITE — SOVEREIGN CLASSIFIED INTRO
 * ===============================================================
...
const BootScreen: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  useEffect(() => {
    particlesRef.current = Array.from({length: 120}, () => ({
      ...
      c: Math.random() > 0.85 ? '#e11d48' : '#be123c' // Rose & Deep Crimson (ELITE)
    }));
  }, []);
  ...
  useEffect(() => {
    ...
    const addLine = () => {
      ...
      setDbLines(p => [...p, line]);
      if (line.includes('ЦІЛЬ ІДЕНТИФІКОВАНО')) setTargetLocked(true);
      idx++;
      const delay =
        line.includes('ВІД НАС НЕ СХОВАЄШСЯ') ? 700 :
        line.includes('ДЕТОНАЦІЇ')        ? 550 :
        ...
        {phase === 0 && (
          <motion.div key="p0" variants={fadeV} initial="initial" animate="animate" exit="exit"
            ...
            <div className="text-center space-y-5">
              ...
              >СУВЕ ЕННЕ  ОЗВІДУВАЛЬНЕ ЯДРО · GLM-5.2 INITIALIZING</motion.div>
            </div>
          </motion.div>
        )}
          ...
          <motion.div key="p2" variants={fadeV} initial="initial" animate="animate" exit="exit"
            ...
            <div className="text-center space-y-2">
              ...
              <div className="text-[7px] text-slate-700 tracking-[0.45em] uppercase">
                47 СУПУТНИКІВ · 892 ВУЗЛИ · 23 ДЕ ЖРЕЄСТРИ · 1.2 PB/ГОД
              </div>
            </div>
          </motion.div>
        )}
