...
export const PermissionLayer: React.FC<PermissionLayerProps> = ({
  ...
  if (!hasAccess) {
    return fallback ? <>{fallback}</> : (
      <div className="flex flex-col items-center justify-center p-16 border-2 border-dashed border-rose-500/30 rounded-[3rem] bg-rose-500/5  relative overflow-hidden group shadow-[inset_0_0_50px_rgba(225,29,72,0.1)]">
  ...
  return (
    <div className="relative group/layer">
       {children}
       {sensitivity !== 'PUBLIC' && (
         <div className="absolute -top-3 -right-3 px-4 py-1.5 bg-black/90  rounded-xl border-2 border-white/10 opacity-0 group-hover/layer:opacity-100 transition-all transform translate-y-2 group-hover/layer:translate-y-0 pointer-events-none shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-50">
            <div className="flex items-center gap-3">
               <div className={cn("w-2 h-2 rounded-full  shadow-[0_0_10px_currentColor]", getBgForSensitivity(sensitivity))} />
