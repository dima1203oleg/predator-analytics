...
export const LivingArchives = () => {
  ...
  useEffect(() => {
    systemApi.getStats()
      .then(res => {
        if (res && typeof res === 'object' && !Array.isArray(res) && 'documents_total' in res) {
          setStats(res);
        } else {
          setStats(null);
        }
  ...
  const totalDocuments = stats && stats.documents_total ? `${(stats.documents_total / 1000000).toFixed(1)}M` : '...';
  ...
  return (
    <div className="flex flex-col font-mono text-xs w-[320px] h-full border border-cyan-tactical/20 bg-obsidian/60 p-4">
      ...
      <div className="text-[10px] text-cyan-tactical/50 mb-4">
        {loading ? t('hud.loading') || 'Завантаження...' : `Всього документів: ${totalDocuments}`}
      </div>
