 ...
 * - Перемикання вкладок (ІНСТРУМЕНТИ, РЕЄСТРИ, СТРІЧКА ПОДІЙ, АНАЛІТИКА)
 * - Пошук реєстрів
...
// ─── Тестові дані ──────────────────────────────────────
const mockTools = [
    { id: 'sherlock', name: 'Sherlock', category: 'СОЦМЕРЕЖІ', status: 'СКАНУЄ', findings: 1420, lastScan: 'Зараз', color: '#a855f7', description: 'Пошук профілів за username', accuracy: 94 },
    { id: 'amass', name: 'Amass', category: 'МЕРЕЖА', status: 'ОНЛАЙН', findings: 872, lastScan: '2хв тому', color: '#3b82f6', description: 'DNS enumeration', accuracy: 97 },
    ...
    categories: [
        {
            ...
            registries: [
                { id: 'edr-1', name: 'Єдиний Державний Реєстр (ЄДР)', status: 'ACTIVE', records: 3200000, lastSync: '2024-03-01', api: 'REST' },
...
describe('OsintCommandCenter', () => {
    ...
    describe('Рендеринг', () => {
        it('повинен показати стан завантаження', () => {
            ...
            expect(screen.getByText(/ІНІЦІАЛІЗАЦІЯ OSINT ЯДРА/i)).toBeInTheDocument();
        });
        ...
        it('повинен відобразити статистику після завантаження', async () => {
            ...
            expect(screen.getByText('ІНСТРУМЕНТІВ')).toBeInTheDocument();
        });
    ...
    describe('Навігація вкладками', () => {
        ...
        it('повинен перемикати на вкладку СТРІЧКА ПОДІЙ', async () => {
            render(<OsintCommandCenter />);
            ...
            fireEvent.click(screen.getByText('СТРІЧКА ПОДІЙ'));

            await waitFor(() => {
                expect(screen.getByText(/СТРІЧКА ПОДІЙ У РЕАЛЬНОМУ ЧАСІ/i)).toBeInTheDocument();
            });
        });
        ...
        it('повинен перемикати на вкладку АНАЛІТИКА', async () => {
            ...
            expect(screen.getByText('КАРТА РИЗИКІВ ЗА ДЖЕРЕЛОМ')).toBeInTheDocument();
    ...
    describe('API інтеграція', () => {
        ...
        it('повинен обробляти помилки API gracefully', async () => {
            ...
            await waitFor(() => {
                expect(screen.queryByText(/ІНІЦІАЛІЗАЦІЯ OSINT ЯДРА/i)).not.toBeInTheDocument();
            });
        });
    });
