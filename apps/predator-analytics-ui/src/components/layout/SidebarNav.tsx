  ...
  return (
    <div className="flex flex-col items-center h-full py-4 gap-4">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* Tooltip (CSS only) */}
              <span className="absolute left-14 bg-black/90 text-xs px-2 py-1 rounded border border-white/10 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap backdrop-blur-md">
                {item.tooltip}
              </span>
            </button>
          );
      ...
      <button 
        ...
        className="w-12 h-12 flex items-center justify-center rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/5 transition-all mt-auto"
        title="Налаштування"
      >
        <Settings size={20} />
      </button>
