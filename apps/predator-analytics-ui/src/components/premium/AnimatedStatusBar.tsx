  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 bg-slate-900/60 backdrop-blur-xl px-4 py-2 rounded-2xl border border-slate-800">
        ...
        <StatusPill
          icon={connectionIcon}
          label={premiumLocales.statusBar.connection}
          value={status.connection === 'online' ? premiumLocales.statusBar.online : premiumLocales.statusBar.offline}
        ...
        <StatusPill
          icon={securityIcon}
          label={premiumLocales.statusBar.security}
          value={status.security === 'secure' ? premiumLocales.statusBar.statusOk : status.security.toUpperCase()}
        ...
        <div className="hidden lg:flex items-center gap-4 px-3 py-1 bg-slate-800/50 rounded-xl border border-slate-700/50">
          <MicroProgressBar value={status.cpu} color="#10b981" label={premiumLocales.statusBar.cpu} />
          <div className="w-px h-4 bg-slate-700" />
          <MicroProgressBar value={status.memory} color="#8b5cf6" label={premiumLocales.statusBar.ram} />
        </div>
          ...
          >
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-500">{premiumLocales.statusBar.uptime}</span>
                <span className="text-xs font-mono text-cyan-400">{status.uptime}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-500">{premiumLocales.statusBar.lastSync}</span>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[10px] text-slate-500 uppercase">{premiumLocales.statusBar.systemHealth}</span>
