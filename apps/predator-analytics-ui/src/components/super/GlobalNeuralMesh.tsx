...
const GlobalNeuralMesh: React.FC<GlobalNeuralMeshProps> = ({ status }) => {
  ...
  return (
    <div className="p-8 bg-slate-950/40 backdrop-blur-3xl border border-yellow-500/10 rounded-[40px] shadow-2xl relative overflow-hidden group h-full flex flex-col">
      {/* Background Grid Decoration */}
      <div className="absolute inset-0 opacity-[0.05] pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle, #D4AF37 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
      ...
      <div className="relative z-10 flex flex-col h-full">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-yellow-600/60 font-black font-mono uppercase tracking-[0.3em]">Distributed Intelligence v56.5-ELITE</p>
            </div>
          </div>
          <div className="flex gap-4 items-center">
            <div className="px-5 py-2 bg-yellow-500/10 border border-yellow-500/20 rounded-full text-[9px] font-black text-yellow-500 font-mono uppercase tracking-widest">
              Active Nodes: 12.4k
            </div>
          </div>
        </div>
        ...
        <div className="mb-8 h-[280px] w-full rounded-[32px] overflow-hidden border border-yellow-500/10 shadow-3xl relative">
            <div className="absolute top-6 left-6 z-40 px-4 py-1.5 bg-yellow-500 text-black text-[10px] font-black rounded-xl uppercase tracking-[0.2em] shadow-lg">
            ...
            {nodes.map((node) => (
              ...
              >
                ...
                <div className={`absolute top-0 right-0 w-32 h-32 blur-[50px] opacity-15 transition-opacity ${
                  ...
                  node.status === 'busy' ? 'bg-rose-500' : 'bg-yellow-500'
                }`} />
                ...
                <div className="flex items-center justify-between mb-5 relative z-10">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-slate-900 flex items-center justify-center border border-white/10 shadow-inner group-hover:bg-slate-850 transition-colors">
                      {node.type === 'nvidia-server' ? <Cpu className="text-yellow-500" size={20} /> :
                ...
                <div className="space-y-4 relative z-10">
                  <div className="flex justify-between items-center text-[10px] uppercase font-black tracking-widest">
                    ...
                    <span className={node.load > 80 ? 'text-rose-400' : 'text-yellow-500'}>{node.load}%</span>
                  </div>
                  <div className="h-2 bg-slate-900 rounded-full overflow-hidden shadow-inner">
                    <motion.div
                      ...
                      className={`h-full bg-gradient-to-r ${
                        node.load > 80 ? 'from-rose-600 to-rose-400' : 'from-yellow-600 to-yellow-400'
                      }`}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-5 py-3 border-t border-white/5">
                    ...
                    {node.gpu_temp && (
                      <div className="flex flex-col items-end">
                        ...
                        <span className="text-[10px] text-yellow-500 font-black tabular-nums">{node.gpu_temp}°C</span>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <button className="w-full mt-8 py-5 bg-yellow-500/5 hover:bg-yellow-500/10 rounded-3xl border border-yellow-500/20 text-[11px] font-black text-yellow-500 uppercase tracking-[0.4em] flex items-center justify-center gap-3 transition-all group hover:tracking-[0.5em]">
          <LinkIcon size={16} className="group-hover:rotate-45 transition-transform" /> 
          ЗАКРІПИТИ НОВИЙ ВУЗОЛ ELITE
        </button>
      </div>
    </div>
  );
