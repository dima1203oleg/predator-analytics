...
export const AutonomousLearningStack: React.FC = () => {
    ...
    return (
        <div className="p-8 bg-slate-950/80 backdrop-blur-3xl border border-amber-500/10 rounded-[40px] shadow-2xl relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
                <div className="flex items-center gap-4">
                    <div className="p-4 bg-amber-500/20 rounded-2xl border border-amber-500/30 shadow-[0_0_20px_rgba(212,175,55,0.15)]">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-amber-600/60 font-black uppercase tracking-[0.3em] font-mono">NEURAL_DEEP_LEARNING_v5.6</p>
                    </div>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 rounded-full border border-amber-500/20">
                    <Activity className="w-3 h-3 text-amber-500" />
                    <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest tabular-nums">1.4B PARAM</span>
                </div>
            </div>

            <div className="space-y-6 relative z-10">
                {/* Current Status */}
                <div className="bg-black/60 p-6 rounded-3xl border border-white/5 group-hover:border-amber-500/30 transition-all duration-500 shadow-inner">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <motion.button
                        ...
                        className="flex items-center justify-center gap-3 p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl hover:bg-amber-500/20 transition-all group/btn disabled:opacity-30 shadow-lg"
                    >
                        <Zap className={`w-4 h-4 text-amber-500 ${isThinking ? 'animate-spin' : 'group-hover/btn:scale-125 transition-transform'}`} />
                        <span className="text-[11px] font-black text-amber-500 uppercase tracking-widest">{premiumLocales.evolution.learningStack.startTraining}</span>
                    </motion.button>
                        ...
                        {Array.isArray(history) && history.slice(0, 3).map((job, i) => (
                            <div key={job.id || i} className="flex items-center justify-between text-[11px] p-3 hover:bg-white/5 rounded-2xl transition-all border border-transparent hover:border-white/10 group/row">
                                <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(212,175,55,0.5)]" />
                                ...
                                <div className="flex gap-3 font-mono items-center">
                                    ...
                                    <span className="text-amber-500/60 font-black tracking-tighter uppercase text-[9px]">{job.status === 'succeeded' ? 'WRAITH' : 'SYNC'}</span>
                                </div>
                            </div>
                        ))}
                    </div>
