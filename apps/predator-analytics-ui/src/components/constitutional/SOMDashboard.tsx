...
export const SOMDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [statusRes, anomaliesRes, proposalsRes, topologyRes, axiomsRes, ledgerRes] = await Promise.all([
        fetch(`${API_BASE}/status`),
        fetch(`${API_BASE}/anomalies?limit=10`),
        fetch(`${API_BASE}/proposals?status=awaiting_approval`),
        fetch(`${API_BASE}/topology`),
        fetch(`${API_BASE}/axioms`),
        fetch(`/api/v1/ledger/entries?limit=5`),
      ]);
      ...
      if (axiomsRes.ok) {
        const data = await axiomsRes.json();
  ...
  return (
    <div className="som-dashboard">
      ...
      <div className="som-grid">
        ...
        >
          <div className="som-card-header">
            <div className="som-card-title">⚖️ Конституційні Аксіоми</div>
          </div>
                ...
                axioms.map(axiom => (
                  <div key={axiom.id} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontWeight: 600, color: '#f8fafc' }}>{axiom.id}</span>
                      <span style={{ fontSize: 11, color: '#94a3b8', background: 'rgba(0,0,0,0.3)', padding: '2px 6px', borderRadius: 4 }}>
                        {axiom.enforcement_level}
                      </span>
                    </div>
                    <div style={{ color: '#cbd5e1' }}>{axiom.description}</div>
                  </div>
                ))
            )}
          </div>
               ...
               <div style={{ textAlign: 'center', color: '#64748b', padding: 20 }}>
                ...
                ledgerEntries.map(entry => (
                  <div key={entry.id} style={{
                    ...
                    <div style={{ color: '#64748b', marginBottom: 2 }}>{new Date(entry.timestamp).toLocaleString()}</div>
                    <div style={{ color: '#e2e8f0', fontWeight: 600 }}>{entry.action_type}</div>
