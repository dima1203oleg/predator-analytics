...
const INITIAL_LOGS: SIContextLog[] = [
    { id: 'init-0', timestamp: '09:59:55', type: 'INFO', source: 'KERNEL', message: 'Booting Singularity Core v25.0...' },
...
export const SuperIntelligenceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [logs, setLogs] = useState<SIContextLog[]>([
        { id: 'boot', timestamp: new Date().toLocaleTimeString(), type: 'INFO', source: 'KERNEL', message: 'Connected to Predator v25 Core. Waiting for stream...' }
    ]);
            ...
            try {
                ...
                if (providers && providers.length > 0) {
                     const realBrainNodes: BrainNodeState[] = providers.map((p: any, idx: number) => ({
                         id: p.id,
                         name: p.name,
                         role: idx === 0 ? 'АРБІТР' : 'ЕКСПЕРТ', // Simple logic
                         avatar: p.name[0],
                         color: p.id === 'google' ? '#3b82f6' : p.id === 'openai' ? '#10b981' : '#a855f7',
                         status: 'IDLE' // could be dynamic if we had usage stats per provider
                     }));
