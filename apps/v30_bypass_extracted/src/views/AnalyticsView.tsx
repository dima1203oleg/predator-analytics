...
// ============================================================================
// ANALYSIS VIEW - СЕМАНТИЧНИЙ РАДАР (v25)
...
const CATEGORY_MAP: Record<string, { color: string; icon: any; label: string }> = {
    PERSON: { color: '#60a5fa', icon: <Info size={14} />, label: 'Особа' },
    ORGANIZATION: { color: '#f59e0b', icon: <Building2 size={14} />, label: 'Організація' },
    LOCATION: { color: '#10b981', icon: <Globe size={14} />, label: 'Локація' },
    PROJECT: { color: '#ec4899', icon: <Target size={14} />, label: 'Проєкт' },
    EVENT: { color: '#ef4444', icon: <Activity size={14} />, label: 'Подія' },
    CONCEPT: { color: '#8b5cf6', icon: <BrainCircuit size={14} />, label: 'Концепт' },
    DEFAULT: { color: '#94a3b8', icon: <Network size={14} />, label: 'Об\'єкт' }
};
    ...
    return (
        <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-[40px]">
            ...
            <motion.div
                ...
                className="absolute top-1/2 left-1/2 w-[200%] h-[200%] -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-blue-500/5 to-transparent origin-center opacity-40"
                style={{ clipPath: 'polygon(50% 50%, 100% 50%, 100% 45%)' }}
            />
...
const AnalyticsView: React.FC = () => {
    ...
    return (
        <div className="min-h-screen flex flex-col h-screen overflow-hidden p-6 gap-6">
            <ViewHeader
                ...
                stats={[
                    { label: 'Вузли', value: summary?.total_nodes?.toString() || '...', icon: <Cpu size={14} />, color: 'primary' },
                    { label: 'Зв\'язки', value: summary?.total_edges?.toString() || '...', icon: <Share2 size={14} />, color: 'success' },
                    { label: 'AI Точність', value: '98.2%', icon: <BrainCircuit size={14} />, color: 'purple' },
                ]}
            />
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center z-20">
                <div className="relative flex-1 max-w-2xl group w-full">
                    ...
                    >
                        {isScanning ? <Activity size={18} className="animate-spin" /> : 'СКАНУВАТИ'}
                    </button>
                </div>

                <div className="flex gap-2">
                    <button className="p-4 bg-slate-900/60 border border-white/10 rounded-2xl text-slate-400 hover:text-white transition-all"><Filter size={20} /></button>
                    <button className="p-4 bg-slate-900/60 border border-white/10 rounded-2xl text-slate-400 hover:text-white transition-all"><Maximize size={20} /></button>
                </div>
            </div>

            {/* MAIN WORKSPACE */}
            <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-4 gap-6 relative">
                    ...
                    <AnimatePresence mode="wait">
                        {isScanning ? (
                            ...
                            >
                                ...
                                <div className="text-[10px] font-black text-blue-400 uppercase tracking-[0.4em] animate-pulse">Глибинне сканування семантичного поля...</div>
                            </motion.div>
                        ) : null}
                    </AnimatePresence>
                ...
                {/* AI Insight Sidebar (Understand -> Act) */}
                <div className="lg:col-span-1 flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <AnimatePresence mode="wait">
                        {selectedEntity ? (
                            ...
                            >
                                {/* Header Card */}
                                <div className="p-6 rounded-3xl bg-slate-900/40 border border-white/5 backdrop-blur-xl relative overflow-hidden group">
                                    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                        {CATEGORY_MAP[selectedEntity.label]?.icon || <Network size={40} />}
                                    </div>
                                    <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: CATEGORY_MAP[selectedEntity.label]?.color }} />
                                        {CATEGORY_MAP[selectedEntity.label]?.label || 'Об\'єкт'}
                                    </div>
                                    <h2 className="text-xl font-black text-white leading-tight mb-4">{selectedEntity.name}</h2>
                                    <div className="flex gap-2">
                                        <button className="flex-1 py-2 bg-blue-600/20 border border-blue-500/20 rounded-xl text-blue-400 text-[10px] font-black uppercase hover:bg-blue-600 hover:text-white transition-all tracking-wider">ПЕРЕГЛЯНУТЬ CASE</button>
                                        <button className="p-2 bg-white/5 rounded-xl hover:bg-white/10 transition-all text-white"><Share2 size={16} /></button>
                                    </div>
                                </div>

                                {/* Properties Grid */}
                                <div className="p-6 rounded-3xl bg-slate-900/40 border border-white/5 backdrop-blur-xl">
                                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">ПАРАМЕТРИ ОБ'ЄКТА</h4>
                                    <div className="space-y-3">
                                        ...
                                        {nodes.length === 0 && <div className="text-xs text-slate-600 italic">Дані відсутні...</div>}
                                    </div>
                                </div>
                                ...
                                <div className="p-6 rounded-3xl bg-gradient-to-br from-indigo-500/10 to-transparent border border-indigo-500/20 backdrop-blur-xl relative">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <span className="text-[10px] font-black text-white uppercase tracking-widest leading-none">AI ІНСАЙТ v25</span>
                                    </div>
                                    <p className="text-xs text-slate-300 leading-relaxed italic">
                                        "Об'єкт {selectedEntity.name} має високу щільність непрямих зв'язків з ризиковими офшорними юрисдикціями через 3-тіх осіб. Рекомендується провести аудит фінансових потоків у секторі {selectedEntity.label}."
                                    </p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 flex flex-col items-center gap-2 hover:bg-emerald-500/10 transition-all cursor-pointer">
                                        ...
                                        <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">БЕЗПЕЧНО</span>
                                    </div>
                                    <div className="p-4 rounded-2xl bg-rose-500/5 border border-rose-500/10 flex flex-col items-center gap-2 hover:bg-rose-500/10 transition-all cursor-pointer">
                                        ...
                                        <span className="text-[9px] font-black text-rose-400 uppercase tracking-widest">РИЗИК</span>
                                    </div>
                                </div>
                            </motion.div>
                        ) : (
                            <div className="flex flex-col items-center justify-center h-full opacity-20 text-slate-500">
                                ...
                                <div className="text-[10px] font-black uppercase tracking-widest">Виберіть вузол для аналізу</div>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
