...
const AgentsView: React.FC = () => {
    ...
    // Truth-only: no simulated resource metrics; rely on backend or empty
    // Simulated resource metrics for visualization
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setResourceData(prev => {
                ...
                const newPoint = {
                    time: (last?.time || 0) + 1,
                    cpu: Math.max(10, Math.min(95, (last?.cpu || 40) + (Math.random() - 0.5) * 30)),
                    mem: Math.max(20, Math.min(80, (last?.mem || 50) + (Math.random() - 0.5) * 10))
                };
