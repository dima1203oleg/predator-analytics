...
const CustomsIntelligenceView: React.FC = () => {
    ...
    const triggerReport = (companyName: string) => {
        ...
        const interval = setInterval(() => {
            current++;
            if (current < l.intel.stages.length) {
                setGeneratingReport(prev => prev ? { ...prev, stage: current + 1 } : null);
            } else {
                    ...
                    >
                        <div className="max-w-xl w-full bg-black/80 border border-white/10 rounded-[64px] p-12 relative overflow-hidden shadow-[0_0_150px_rgba(245,158,11,0.1)]">
                            ...
                            <div className="text-center space-y-8">
                                ...
                                {dossierReady && (
                                    <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="pt-4 flex gap-4">
                                        <button className="flex-1 py-5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-black rounded-3xl uppercase text-[10px] tracking-[0.2em] shadow-xl hover:scale-105 transition-all flex items-center justify-center gap-3">
                                            <Download size={18} /> {l.intel.downloadPdf}
                                        </button>
                                        <button onClick={() => setGeneratingReport(null)} className="px-8 py-5 bg-white/5 border border-white/10 text-slate-400 font-black rounded-3xl uppercase text-[10px] hover:bg-white/10 transition-all">
                                            {l.intel.dismiss}
                                        </button>
                                    </motion.div>
                                )}
