      ...
      if (lastEvent.type === 'etl_update') {
        switch (lastEvent.event) {
          case 'ingestion_started':
            toast.info('Neural Ingestion Started', `Ingesting data from: ${lastEvent.source}`);
            break;
          case 'ingestion_completed':
            toast.success('Neural Ingestion Completed', `Successfully fetched ${lastEvent.records_fetched} records from ${lastEvent.source}`);
            break;
          case 'processing_started':
            toast.info('Semantic Processing Started', `Deep analyzing ${lastEvent.records_count} records in cortex`);
            break;
          case 'processing_completed':
            toast.success('Semantic Processing Completed', `Analysis finished. ${lastEvent.processed_count} gold documents generated.`);
            break;
          case 'indexing_started':
            toast.info('Vector Indexing Started', `Indexing ${lastEvent.count} documents to neural storage`);
            break;
          case 'indexing_completed':
            toast.success('Vector Indexing Completed', `System synchronised. Indexed in: OpenSearch (${lastEvent.indexed_opensearch}), Qdrant (${lastEvent.indexed_qdrant})`);
            break;
        }
      }
    }
