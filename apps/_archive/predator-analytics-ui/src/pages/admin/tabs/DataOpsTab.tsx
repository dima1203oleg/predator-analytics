...
const kafkaCols: VirtualColumn<KafkaTopic>[] = [
  ...
  { key: 'partitions', label: 'ПАРТ.', width: '65px',  mono: true, align: 'right' },
  {
    ...
    render: (v) => {
      const n = Number(v);
      return <span className={cn('font-black italic glint-elite', n > 5000 ? 'text-cyan-500' : n > 500 ? 'text-amber-500' : 'text-emerald-500/80')}>{n.toLocaleString()}</span>;
    },
  },
  ...
  { key: 'consumers',  label: 'КОНС.',   width: '65px', mono: true, align: 'right' },
  {
    ...
    render: (v) => {
      const s = String(v);
      const map: Record<string, string> = { ok: 'text-cyan-500 shadow-cyan-500/20', warn: 'text-amber-500 shadow-amber-500/20', error: 'text-cyan-600 shadow-rose-600/20' };
...
const datasetCols: VirtualColumn<DatasetRecord>[] = [
  ...
  { key: 'sizeGb',    label: 'ГБ',       width: '80px',  mono: true, align: 'right', render: (v) => <span className="text-cyan-500/60 font-black italic">{Number(v).toFixed(1)}</span> },
  { key: 'version',   label: 'ВЕРСІЯ',   width: '90px',  mono: true, render: (v) => <span className="text-white/20 font-black italic">v{String(v)}</span> },
  {
    ...
    render: (v) => {
      const s = String(v);
      const map: Record<string, string> = { ready: 'text-cyan-500', training: 'text-rose-400', outdated: 'text-amber-400', draft: 'text-white/30' };
...
const moduleCols: VirtualColumn<FactoryModule>[] = [
  ...
  { key: 'template',  label: 'ШАБЛОН_КОНВЕЄРА',    width: '220px', mono: true, render: (v) => <span className="text-white/35 text-[10px] italic font-black uppercase tracking-tight">{String(v)}</span> },
  {
    ...
    render: (v) => {
      const s = String(v);
      const map: Record<string, string> = { deployed: 'text-cyan-500', pending: 'text-rose-400/70', failed: 'text-cyan-600', draft: 'text-white/30' };
...
export const DataOpsTab: React.FC = () => {
      ...
      <div className="flex flex-col items-center justify-center h-[700px] text-white/30 space-y-12 relative overflow-hidden">
        ...
        <div className="relative">
          <motion.div 
            ...
            className="w-24 h-24 border-4 border-cyan-500/10 rounded-full border-t-rose-500 "
          />
          <Database className="absolute inset-0 m-auto w-8 h-8 text-cyan-500 " />
        </div>
        <div className="flex flex-col items-center gap-4">
           <div className="text-[14px] font-mono uppercase tracking-[0.8em]  italic font-black text-cyan-500 glint-elite">ІНТЕР-ОПЕРАБЕЛЬНІСТЬ_ДАНИХ...</div>
  ...
  if (isError || !data) {
    return (
      <div className="flex flex-col items-center justify-center h-[700px] p-24 text-center glass-wraith m-12 border-2 border-cyan-600/20 rounded-[4rem] relative overflow-hidden shadow-4xl">
        ...
        <Database size={80} className="text-cyan-600/40 mb-12 " />
        ...
        <p className="text-[14px] font-black text-white/30 max-w-2xl mb-16 leading-relaxed uppercase italic tracking-widest">
          ...
          ПЕРЕВІРТЕ З'ЄДНАННЯ З КОНТРОЛЕРОМ ДАНИХ <span className="text-cyan-500">PREDATOR_LAKE_MASTER_L7</span>.
        </p>
        <Button variant="cyber" 
           onClick={() => window.location.reload()}
           className="px-16 py-7 bg-cyan-600 text-white text-[12px] font-black uppercase tracking-[0.6em] rounded-[2rem] hover:bg-cyan-500 transition-all shadow-4xl italic border-2 border-cyan-400/50"
        >
  ...
  return (
    <div className="p-16 space-y-20 max-w-[1800px] mx-auto relative pb-40">
      {/* Header Section */}
      <div className="flex flex-col lg:flex-row items-center justify-between gap-12 border-l-4 border-cyan-600 pl-14 py-4 relative overflow-hidden group">
        <div className="absolute inset-0 bg-cyan-500/5 blur-[60px] -translate-x-full group-hover:translate-x-full transition-transform duration-[2000ms] pointer-events-none" />
        <div className="space-y-5">
           <div className="flex items-center gap-8">
             <h2 className="text-5xl font-black text-white uppercase tracking-tighter italic glint-elite chromatic-elite leading-tight">
               УПРАВЛІННЯ ДАНИМИ & <span className="text-cyan-500">ПОТОКОВА АНАЛІТИКА</span>
             </h2>
             <div className="px-5 py-1.5 bg-cyan-600/10 border-2 border-cyan-600/30 rounded-xl text-[11px] font-black text-cyan-500 tracking-[0.4em] uppercase italic shadow-2xl">
               ELITE_DATA_v61.0
             </div>
           </div>
        ...
        <div className="flex items-center gap-6">
           <div className="p-4 bg-cyan-600/10 border-2 border-cyan-600/20 rounded-[2rem] shadow-4xl group/icon hover:border-cyan-500/40 transition-all duration-700">
              <Orbit size={48} className="text-cyan-500 animate-spin-slow" style={{ animationDuration: '20s' }} />
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        {[
          ...
          { label: 'ЗАГАЛЬНА_ЧЕРГА', value: kafkaTopics.reduce((s,t)=>s+t.lag,0).toLocaleString(), color: 'text-cyan-500', sub: 'ЗАПИСІВ_У_ЧЕРЗІ_ОЧІКУВАННЯ', icon: Activity },
          ...
          <motion.div 
            ...
            className="glass-wraith border-2 border-white/5 p-12 rounded-[3.5rem] group hover:border-cyan-500/40 transition-all duration-1000 relative overflow-hidden shadow-4xl hover:-translate-y-2"
          >
            ...
            <div className="absolute top-8 right-10 opacity-5 group-hover:opacity-25 transition-all duration-1000 transform group-hover:scale-125">
               <m.icon size={80} className="text-cyan-500" />
            </div>
            <div className="text-[11px] font-black font-mono text-white/20 uppercase tracking-[0.5em] mb-6 italic group-hover:text-cyan-500/40 transition-colors duration-700">{m.label}</div>
            <div className={cn('text-6xl font-black tracking-tighter italic leading-none glint-elite', m.color)}>{m.value}</div>
            <div className="text-[10px] font-black font-mono text-white/10 mt-10 uppercase tracking-[0.4em] font-bold group-hover:text-cyan-500/60 transition-colors italic border-l-2 border-white/5 pl-4">{m.sub}</div>
            <div className="absolute bottom-8 right-10 w-12 h-[2px] bg-white/5 group-hover:bg-cyan-500/60 transition-colors duration-700" />
          </motion.div>
        ))}
      </div>
          ...
          return (
            <Button variant="cyber"
              ...
              className={cn(
                ...
                active
                  ? 'bg-cyan-600/10 border-cyan-500/40 shadow-cyan-500/20 scale-[1.03]'
            ...
            >
              ...
              <div className="flex items-center gap-8 w-full">
                <div className={cn(
                   ...
                   active ? 'bg-cyan-600 text-white ' : 'bg-white/5 text-white/20 group-hover:text-white/40'
                )}>
                ...
                <div className="flex-1 text-left space-y-2">
                   ...
                   <div className="text-[11px] font-black font-mono text-white/10 uppercase tracking-[0.3em] italic group-hover:text-cyan-500/20 transition-colors">{t.count} ОБ'ЄКТІВ_ЯДРА</div>
                </div>
                {active && (
                   <motion.div 
                     layoutId="data-tab-indicator-v61"
                     className="w-2 h-12 bg-cyan-600 rounded-full "
                   />
                )}
              </div>
            </Button>
      ...
      <motion.div
        ...
        className="glass-wraith border-2 border-white/5 rounded-[5rem] overflow-hidden  shadow-4xl relative p-8 group/content hover:border-cyan-500/20 transition-all duration-1000"
      >
        <div className="absolute inset-0 bg-cyber-grid opacity-[0.02] pointer-events-none" />
        <div className="absolute top-6 left-12 flex items-center gap-6 text-[11px] font-black text-white/20 uppercase tracking-[0.6em] italic mb-8 relative z-10">
           <Terminal size={18} className="text-cyan-600" /> ТАБЛИЦЯ_ОБРОБКИ_ДАТА_ОПС_L7
        </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-12 opacity-40 hover:opacity-100 transition-opacity duration-[2000ms] px-10">
        <div className="flex items-center gap-6 px-10 py-5 bg-cyan-600/5 border-2 border-cyan-500/10 rounded-[2.5rem] shadow-4xl group hover:border-cyan-500/30 transition-all">
           <TrendingUp className="w-6 h-6 text-cyan-500 group-hover:scale-125 transition-transform" />
           <span className="text-[13px] font-mono text-cyan-500 font-black uppercase tracking-[0.5em] italic glint-elite">ЯДРО_ДАТА_КОНВЕЄРА_СТАБІЛЬНЕ_v61.0</span>
        </div>
        ...
        <div className="flex flex-col items-end gap-2 text-right">
           ...
           <span className="text-[10px] font-black font-mono text-cyan-600/40 uppercase tracking-[0.4em] italic">ELITE_STABILITY_PROTOCOL_READY</span>
        </div>
      </div>
