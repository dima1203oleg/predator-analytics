...
export const RealTimeMonitor: React.FC = () => {
  ...
  const getEventIcon = (type: string) => {
    switch (type) {
      ...
      case 'deleted': return <ShieldAlert className="text-cyan-500" />;
    ...
    <PageTransition>
      <div className="min-h-screen bg-black text-slate-200 relative overflow-hidden font-sans pb-40">
        ...
        <div className="relative z-10 max-w-[1850px] mx-auto px-10 pt-16 space-y-20">
            ...
            title={
              <div className="flex items-center gap-12">
                <div className="relative group">
                  ...
                  <div className="relative p-10 bg-black/60  border-2 border-cyan-500/30 rounded-[3.5rem] shadow-4xl transform -rotate-3 group-hover:rotate-0 transition-all duration-700">
                    <Radio size={56} className="text-cyan-500 " />
                  </div>
                </div>
                <div className="space-y-5">
                  ...
                  <h1 className="text-8xl font-black text-white tracking-tighter uppercase italic leading-none glint-elite chromatic-elite">
                    МОНІТОРИНГ <span className="text-cyan-500">ПОДІЙ</span>
                  </h1>
                  <p className="text-[13px] text-white/40 font-black uppercase tracking-[0.6em] italic leading-none border-l-4 border-cyan-900/50 pl-8">
          ...
          <div className="grid grid-cols-12 gap-16">
            ...
            <div className="col-span-12 xl:col-span-3 space-y-12">
               <div className="relative p-12 rounded-[4rem] glass-wraith border-2 border-white/5 shadow-4xl space-y-12 overflow-hidden group hover:border-cyan-500/20 transition-all duration-1000">
                  <div className="absolute inset-0 bg-cyber-grid opacity-[0.02] pointer-events-none" />
                  <div className="flex items-center gap-8 border-b-2 border-white/5 pb-10">
                     <div className="p-4 bg-cyan-600/10 rounded-2xl border-2 border-cyan-500/20 shadow-2xl">
                        <Filter size={28} className="text-cyan-500 glint-elite" />
                     </div>
                  ...
                  <div className="space-y-10 relative z-10">
                     <div className="space-y-5">
                        ...
                        <select
                          ...
                          className="w-full bg-white/[0.03] border-2 border-white/5 p-6 rounded-[2rem] text-[12px] font-black text-white/80 italic tracking-widest outline-none focus:border-cyan-500/40 transition-all duration-700 cursor-pointer shadow-inner hover:bg-white/[0.06]"
                        >
                  ...
                  <div className="absolute -bottom-16 -right-16 w-64 h-64 bg-cyan-500/5 blur-[80px] rounded-full group-hover:bg-cyan-500/15 transition-all duration-1000" />
               </div>

               <div className="p-12 rounded-[4rem] glass-wraith border-2 border-cyan-600/10 space-y-10 relative overflow-hidden group hover:border-cyan-500/40 transition-all duration-1000">
                  <div className="absolute inset-0 bg-cyber-grid opacity-[0.03] pointer-events-none" />
                  <div className="flex items-center gap-8">
                     <div className={cn(
                       "w-4 h-4 rounded-full shadow-[0_0_20px_currentColor] transition-all duration-1000", 
                       isConnected ? "bg-emerald-500  shadow-emerald-500/60" : "bg-cyan-500 shadow-cyan-500/60"
                     )} />
                  ...
                  >
                    <RefreshCw size={180} className="text-cyan-500" />
                  </motion.div>
               </div>
            </div>
            ...
            <div className="col-span-12 xl:col-span-9 space-y-12">
               <div className="flex items-center justify-between px-12">
                  <h4 className="text-[14px] font-black text-white/30 uppercase tracking-[0.6em] italic flex items-center gap-8">
                     <Terminal size={24} className="text-cyan-600 glint-elite" /> ТЕРМІНАЛ_АКТИВНОСТІ_ЯД А_OODA
                  </h4>
                  <div className="flex items-center gap-8">
                      ...
                      >
                         <div className="relative">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                               <Activity size={48} className="text-cyan-600/40 " />
                            </div>
                         </div>
                      ...
                      filteredEvents.map((event, idx) => (
                        <motion.div
                          ...
                          className={cn(
                            ...
                            event.severity === 'critical' ? "border-rose-950/60 hover:border-cyan-600/60 shadow-rose-950/20" : 
                        ...
                        >
                           ...
                           <div className="flex items-center justify-between gap-16 relative z-10 mt-6">
                              <div className="flex items-center gap-14">
                                 <div className={cn(
                                   ...
                                   event.severity === 'critical' ? "bg-cyan-600/10 border-cyan-500 text-cyan-500 shadow-cyan-500/30" : 
                                 ...
                                 <div className="space-y-5">
                                    <div className="flex items-center gap-8">
                                       <h3 className="text-5xl font-black text-white italic tracking-tighter uppercase leading-none glint-elite group-hover:text-cyan-500 transition-colors duration-700">{event.company.name}</h3>
                                       <span className="px-6 py-2 bg-white/5 border-2 border-white/10 rounded-2xl text-[12px] font-black text-white/40 italic uppercase tracking-widest shadow-inner group-hover:border-cyan-500/20 transition-all">{event.company.ueid}</span>
                                    </div>
                                    <div className="flex items-center gap-10 text-[12px] font-black text-white/20 uppercase italic tracking-[0.3em]">
                                       <div className="flex items-center gap-4 group-hover:text-cyan-500/60 transition-colors"><MapPin size={18} className="text-cyan-600" /> {event.company.region}</div>
                                       <div className="w-2 h-2 rounded-full bg-white/5 shadow-inner" />
                                       <div className="flex items-center gap-4 group-hover:text-cyan-500/60 transition-colors"><Clock size={18} className="text-cyan-600" /> {new Date(event.timestamp).toLocaleTimeString('uk-UA')}</div>
                                       <div className="w-2 h-2 rounded-full bg-white/5 shadow-inner" />
                              ...
                              <div className="flex flex-col items-end gap-5 text-right">
                                 <div className={cn(
                                   ...
                                   event.severity === 'critical' ? "bg-cyan-600/10 border-cyan-600/50 text-cyan-500 shadow-cyan-500/30" : 
                                 ...
                                 <div className="text-[11px] font-black text-white/10 uppercase italic tracking-widest font-mono group-hover:text-cyan-500/40 transition-colors duration-700">OSINT_TRACE_ID: {event.id.slice(0, 16).toUpperCase()}</div>
                              </div>
                           </div>
                           ...
                           {event.changes && Object.keys(event.changes).length > 0 && (
                             <div className="mt-14 pt-14 border-t-2 border-white/5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 relative z-10">
                                {Object.entries(event.changes).map(([key, value]) => (
                                  <div key={key} className="p-8 rounded-[2.5rem] bg-white/[0.02] border-2 border-white/5 space-y-3 group/field hover:bg-cyan-500/5 hover:border-cyan-500/20 transition-all duration-700 shadow-inner">
                                     <p className="text-[10px] font-black text-white/20 uppercase italic tracking-[0.4em] group-hover/field:text-cyan-500/60 transition-colors duration-700">{key.toUpperCase()}</p>
                                     <p className="text-lg font-black text-white italic truncate tracking-tight glint-elite group-hover/field:text-cyan-500 transition-colors">{String(value)}</p>
                                  </div>
                                ))}
                             </div>
                           )}
                           ...
                           <div className="absolute bottom-6 right-16 flex gap-6 opacity-0 group-hover:opacity-100 transition-all duration-700 translate-x-10 group-hover:translate-x-0">
                              <Button variant="cyber" className="flex items-center gap-3 text-[10px] font-black text-cyan-500 uppercase tracking-widest italic hover:text-white transition-colors">
        ...
        <style dangerouslySetInnerHTML={{ __html: `
            ...
            .glint-elite { text-shadow: 0 0 30px rgba(6,182,212,0.3); }
