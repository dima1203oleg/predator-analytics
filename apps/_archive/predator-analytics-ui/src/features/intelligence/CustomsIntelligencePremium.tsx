    ...
    <PageTransition>
      <div className="min-h-screen bg-[#020202] text-slate-200 relative overflow-hidden font-sans pb-40 px-4 xl:px-12">
              ...
              <div className="flex items-center gap-12">
                <div className="relative group">
                  ...
                  <div className="relative p-8 bg-black border-2 border-cyan-500/40 rounded-[3rem] shadow-4xl transform -rotate-3 hover:rotate-0 transition-all duration-700">
                    <Crown size={48} className="text-cyan-500 shadow-[0_0_30px_#e11d48]" />
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex items-center gap-6">
                    <span className={cn(
                      ...
                      isOffline ? "bg-cyan-500/10 border-cyan-500/20 text-cyan-500" : "bg-cyan-600/10 border-cyan-600/20 text-cyan-500"
                    )}>
                  ...
                  <h1 className="text-7xl font-black text-white tracking-tighter uppercase italic skew-x-[-4deg] leading-none">
                    МИТНИЙ <span className={cn("underline decoration-[16px] underline-offset-[16px] italic uppercase tracking-tighter", isOffline ? "text-cyan-500 decoration-rose-500/20" : "text-cyan-600 decoration-rose-600/30")}>PROJECT</span>
                  </h1>
                </div>
              </div>
            }
          ...
          <div className="grid grid-cols-12 gap-12">
            ...
            <div className="col-span-12 xl:col-span-8 space-y-12">
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {[
                    { l: 'ЗАГАЛЬНИЙ_ІМПОРТ', v: '$1.42B', c: '+12.4%', i: Package, cl: 'text-cyan-500' },
               ...
               <div className="p-12 rounded-[4rem] bg-black border-2 border-white/[0.04] shadow-3xl space-y-10">
                  <div className="flex items-center justify-between pb-8 border-b border-white/[0.04]">
                     <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase flex items-center gap-6">
                        <Users className="text-cyan-500" size={32} /> ТОП ІМПОРТЕ ІВ СЕГМЕНТУ
                     </h3>
                     <Button variant="cyber" className="text-cyan-500 text-[10px] font-black uppercase tracking-widest italic flex items-center gap-3 hover:gap-5 transition-all">
                        ПОВНИЙ СПИСОК <ChevronRight size={16} />
                     </Button>
                  </div>
                  <div className="space-y-6">
                     {TOP_IMPORTERS.map((comp, i) => (
                       <div key={i} className="p-8 rounded-[2.5rem] bg-white/[0.01] border-2 border-white/[0.04] hover:border-cyan-500/20 transition-all group flex items-center justify-between">
                          <div className="flex items-center gap-8">
                             <div className="w-16 h-16 rounded-3xl bg-black border border-white/10 flex items-center justify-center text-slate-600 group-hover:text-cyan-500 transition-colors">
                                <Building2 size={24} />
                             </div>
                             <div className="space-y-1">
                                <h4 className="text-xl font-black text-white italic tracking-tighter uppercase group-hover:text-cyan-500 transition-colors leading-none">{comp.name}</h4>
            ...
            <div className="col-span-12 xl:col-span-4 space-y-10">
               <div className="p-10 rounded-[4rem] bg-black border-2 border-cyan-500/10 shadow-3xl flex flex-col gap-10">
                  <h3 className="text-2xl font-black text-white italic tracking-tighter uppercase flex items-center gap-6">
                     <Sparkles className="text-cyan-500" size={32} /> ШІ-АНАЛІЗ  ИНКУ
                  </h3>
                  <div className="space-y-6">
                     {[
                       ...
                       <div key={i} className="p-6 rounded-3xl bg-white/[0.01] border-2 border-white/[0.04] space-y-3">
                          <div className="flex items-center justify-between">
                             <span className="text-[10px] font-black text-cyan-500 uppercase tracking-widest italic">{insight.t}</span>
                             <span className={cn("w-2 h-2 rounded-full", insight.r === 'high' ? 'bg-cyan-500 shadow-[0_0_8px_#e11d48]' : insight.r === 'medium' ? 'bg-cyan-600' : 'bg-emerald-500')} />
                          </div>
                          <p className="text-[11px] text-slate-400 italic font-medium leading-relaxed uppercase">{insight.d}</p>
                       </div>
                     ))}
                  </div>
                  <Button variant="cyber" className="w-full py-6 bg-cyan-600 hover:bg-cyan-500 text-black rounded-[2rem] text-[11px] font-black uppercase tracking-[0.3em] font-serif italic shadow-4xl transition-all">
               ...
               <div className="p-10 rounded-[4rem] bg-black border-2 border-white/[0.04] shadow-3xl text-center space-y-6">
                  <div className="mx-auto w-24 h-24 bg-cyan-500/5 border border-cyan-500/30 rounded-full flex items-center justify-center mb-4">
        ...
        <style dangerouslySetInnerHTML={{ __html: `
            ...
            .shadow-4xl { box-shadow: 0 40px 80px -20px rgba(6,182,212,0.3); }
        `}} />
      </div>
    </PageTransition>
  );
