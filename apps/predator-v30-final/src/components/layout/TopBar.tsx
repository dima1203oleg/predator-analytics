...
import { Monitor, Smartphone, Tablet, Search, ExternalLink, BarChart3, Database, Menu, ShieldAlert, Cpu } from 'lucide-react';
import { cn } from '../../utils/cn';
import OperatorIdentity from './OperatorIdentity';
import { AnimatedStatusBar } from '../premium/AnimatedStatusBar';
...
export const TopBar = () => {
  ...
  return (
    <div className="h-16 border-b border-white/5 bg-slate-950/80 backdrop-blur-xl flex items-center justify-between px-6 fixed top-0 left-0 right-0 z-50 shadow-2xl shadow-black/50">

      {/* Left: Logo & Sidebar Toggle */}
      <div className="flex items-center gap-6">
        <button onClick={toggleSidebar} className="p-2 hover:bg-slate-800 rounded-lg lg:hidden">
          ...
          <div className="hidden sm:block">
            ...
            <div className="flex items-center gap-2 mt-1">
               ...
               <div className="flex items-center gap-1">
                 ...
                 <span className="text-[9px] text-emerald-500/80 font-mono">ONLINE</span>
               </div>
            </div>
          </div>
        </div>
      ...
      <div className="hidden 2xl:flex items-center gap-8 bg-black/20 border border-white/5 px-6 py-2 rounded-2xl backdrop-blur-sm">
         <div className="flex items-center gap-3">
            ...
            <div>
                <div className="text-[9px] text-slate-500 font-black uppercase tracking-widest">THREAT LEVEL</div>
         ...
         <div className="flex items-center gap-3">
             ...
             <div>
                 <div className="text-[9px] text-slate-500 font-black uppercase tracking-widest">SYSTEM INTEGRITY</div>
                 <div className="text-xs font-mono font-bold text-blue-400">100% OPTIMAL</div>
             </div>
         </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        ...
        {/* Role Switcher - TELEMETRY STYLE */}
        <div className="hidden md:flex items-center gap-2 bg-slate-950/40 p-1.5 rounded-2xl border border-white/5 backdrop-blur-md">
          {['client', 'premium', 'admin'].map((role) => (
             <button
                key={role}
                onClick={() => setRole(role as any)}
                className={cn(
                    "px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all relative overflow-hidden group/role",
                    userRole === role
                        ? role === 'admin' ? "text-red-400 bg-red-500/10 border border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.1)]" :
                          role === 'premium' ? "text-amber-400 bg-amber-500/10 border border-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.1)]" :
