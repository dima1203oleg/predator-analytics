...
export const LiveDeploymentColumn: React.FC<LiveDeploymentColumnProps> = ({ env, color }) => {
    ...
    return (
        <div className={`flex flex-col h-full bg-slate-900/50 border rounded-xl  panel-3d ${c.border} ${env.status === 'DEGRADED' ? 'border-red-500/50' : ''}`}>
            {/* Header */}
            <div className={`p-4 border-b ${c.border} bg-slate-950/30 relative `}>
                ...
                <div className="mt-1 w-full h-1 bg-slate-800 rounded-full ">
            ...
            <div className="h-32 bg-[#050a14] border-t border-slate-800 p-2 font-mono text-[9px] overflow-y-auto custom-scrollbar relative">
                <div className="absolute top-1 right-2 text-slate-600 uppercase font-bold text-[8px]">Живий Лог</div>
                <div className="space-y-1">
                    {logs.map((log, i) => (
